...
sam_3D_dir = '/home/weberjk/Plot_CRM/SAM_LBA/Data/3D/1km/new_sim/'
...
sam_3D_file = 'LBA_128kmx128kmx128_1km_Morrison_64_'
...
out_dir = '/home/weberjk/Ticket764/output/%s/'%(case_dir) # Figure output directory
                      # and and name

plot_var_name = '$\chi$'         # How the variable looks when it is plotted
plot_var_units = '$kg\ kg^{-1}$' # The plotted units
                      
z0_array = [1000,1500,2000,2500,3000]               # Analysis height [m]
...
for i in np.arange(0,len(z0_array)):
    z0 = z0_array[i]
    out_name = 'lba_%s_%sm_hist_chi.png'%(case_dir, z0)    
    ...
    sam_stat = describe(sam_chi)
    clb_stat = describe(clb_chi)
    
    sam_text = 'SAM \n $\mu$ = %.2e \n $\sigma^{2}$ = %.2e \n $Sk$ = %.2e'%(sam_stat[2],sam_stat[3],sam_stat[4])
    clb_text = 'CLUBB \n $\mu$ = %.2e \n $\sigma^{2}$ = %.2e \n $Sk$ = %.2e'%(clb_stat[2],clb_stat[3],clb_stat[4])
    ...
    plt.grid()
    plt.legend()
    ax1.text(0.05, 0.95, sam_text, transform=ax1.transAxes, fontsize=14, verticalalignment='top', bbox=props)
    ax1.text(0.05, 0.70, clb_text, transform=ax1.transAxes, fontsize=14, verticalalignment='top', bbox=props)

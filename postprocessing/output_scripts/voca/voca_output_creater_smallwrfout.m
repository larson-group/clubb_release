...
%   voca_output_creator( infile_name, action, crs_lat, tstart_avg, tend_avg )
...
%   UPDATE: The Definition and the Output File section are now encapsulated in the function 
%   'write_output'. Furthermore the main function requires a string specifying the action 
%   of this script after reading the data.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for ifilenum=1:numfiles
    ...
    if ifilenum==1
      ...
      theta=zeros(length(voca_lons),length(voca_lats),bottom_top,calc_times);
      press=theta; qvapor=theta; qcloud=theta; qrain=theta; cf=theta;
    ...
    clear T P PB QVAPOR QCLOUD QRAIN CF_CLUBB % for space
    disp('Done interpolating unstaggered 3-D variables to VOCA domain for file:'); %disp(ifilenum);
    
    if ifilenum==1
      landmask=zeros(length(voca_lons),length(voca_lats),calc_times);
    end
...
% This function writes the netcdf output for the intercomparison.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % TODO: We may have to do adjustments here for the next intercomparison.
...
% Creates plots similar to these in Rob Wood's talk (see wrf:ticket:47)
 ...
 % calculate rain water path and cloud cover
  ...
  cf_t_avg = calc_time_avg_3d(cf_int, Times, tstart_avg, tend_avg);
  ...
  cloudwater_t_avg = calc_time_avg_3d(cloudwater_int, Times, tstart_avg, tend_avg);
  ...
  qv_t_avg = calc_time_avg_3d(qv_int, Times, tstart_avg, tend_avg);
...
% ATTENTION: This function does not work. It is only copied from Leah Grant's files. It can be
% considered as a reminder to make it work.
...
end % function mix_rat_to_spec_humid
        ...
        elseif pressure(i,j,1,n) == 85000.
          ...
          for k=2:size(var,3)
            ...
            elseif pressure(i,j,k,n) < 85000.
              ...
              break
            end % pressure(i,j,k,n) == 85000.

          end % for k=2:size(var,3)

        end % if pressure(i,j,1,n) < 85000.

      end
    end
  end

end % function interp_to_850mb
...
end % function calc_pressure_staggered
      ...
      for n=1:size(cloud_frac,4)
        ...
        if pressure(i,j,1,n) > 70000. % Pa ; skip this column if psfc<700mb since already set low_cloud_frac to zeros
          ...
          for k=1:size(cloud_frac,3)
            ...
            end % if low_cloud_frac(i,j,n) < cloud_frac(i,j,k,n)

          end % for k=1:size(cloud_frac,3)

        end % if pressure(i,j,1,n) > 70000.

      end
    end
  end

end % function calc_low_cloud_frac
...
end % function convert_precip_flux
...
end % function fill_miss_values_ND_array
...
end % function calc_t_p_nd_cldtop
...
end % function fill_miss_values_from_lowcld_frac



function mtx_t_averaged = calc_time_avg_3d( mtx, Times, tstart_avg, tend_avg)

%   mtx_t_averaged = sum(mtx, t_dim)/size(mtx, t_dim);
  mtx_t_averaged = zeros(size(mtx,1), size(mtx,2));
  i_between = get_times_between( Times(12:end, :), tstart_avg, tend_avg );
  
  for i = 1:size(mtx, 1)
    for j = 1:size(mtx, 2)
      
      mtx_t_averaged(i,j) = sum(mtx(i,j,i_between))/size( i_between, 2 );
      
%       count = 0;
%       for k = 1:size(mtx, 3) % time loop
%         
%         if is_time_between(Times(12:end, k), tstart_avg, tend_avg)
%           mtx_t_averaged(i,j) = mtx_t_averaged(i,j) +  mtx(i,j,k);
%           count = count + 1;
%         end
%         
%         mtx_t_averaged(i,j) = mtx_t_averaged(i,j)/count;
%         
%       end % time loop
    end
  end
     
end % function calc_time_avg
...
end % function flip_var_for_plot
...
function [ hours, minutes, seconds ] = parse_time( time_string )
% Parses a time string (hh:mm:ss) to integer values

  hours = str2double(time_string(1:2));
  minutes = str2double(time_string(4:5));
  seconds = str2double(time_string(7:8));

end

function l_between = is_time_between( t_str_check, t_str_start, t_str_end )
% Returns 1 if t_str_check is in the time interval [t_str_start, t_str_end]
% 0 otherwise.
  
  l_between = 0;

  [ht1 mt1 st1] = parse_time(t_str_start);
  [ht2 mt2 st2] = parse_time(t_str_end);
  [ht3 mt3 st3] = parse_time(t_str_check);  
  
  t_num_start = ht1*10^4+mt1*10^2+st1;
  t_num_end = ht2*10^4+mt2*10^2+st2;
  t_num_check = ht3*10^4+mt3*10^2+st3;
  
  if t_num_check >= t_num_start && t_num_check <= t_num_end
    l_between = 1;
  end
...
function ind_between = get_times_between( t_array, t_str_start, t_str_end )

  count = 1;
  for k = 1:size(t_array, 2)
    if ( is_time_between( t_array(:,k), t_str_start, t_str_end ) )
      ind_between(count) = k;
      count = count + 1;
    end
  end

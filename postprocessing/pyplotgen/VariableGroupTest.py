...
class VariableGroupBase(VariableGroup):
    ...
    def __init__(self, ncdf_files, sam_file = None):
        ...
        thlm_plots = super().getVarLinePlots('thlm', ncdf_files, averaging_start_time=self.averaging_start_time, averaging_end_time=self.averaging_end_time, label="current clubb")
        if sam_file != None:
            thlm_sam = self.getThlmSamPlot()
            thlm_plots.append(thlm_sam)

        rtm_plots = super().getVarLinePlots('rtm', ncdf_files, averaging_start_time=self.averaging_start_time, averaging_end_time=self.averaging_end_time, label="current clubb")
        if sam_file != None:
        ...
        wpthlp_plots = super().getVarLinePlots('wpthlp', ncdf_files, averaging_start_time=self.averaging_start_time, averaging_end_time=self.averaging_end_time, sam_file=sam_file, label="current clubb")
        wprtp_plots = super().getVarLinePlots('wprtp', ncdf_files, averaging_start_time=self.averaging_start_time, averaging_end_time=self.averaging_end_time, sam_file=sam_file, label="current clubb")
        wp2_plots = super().getVarLinePlots('wp2', ncdf_files, averaging_start_time=self.averaging_start_time, averaging_end_time=self.averaging_end_time, sam_file=sam_file, label="current clubb")
        den_wgt_avg_plots = super().getVarLinePlots('wp2_vert_avg', ncdf_files['sfc'], averaging_start_time=self.averaging_start_time,
        ...
        cloud_frac_plots = super().getVarLinePlots('cloud_frac', ncdf_files, sam_conv_factor=100, conversion_factor=100, averaging_start_time=self.averaging_start_time, averaging_end_time=self.averaging_end_time, sam_file=sam_file, label="current clubb")
        ...
        den_wgt_avg = Panel(den_wgt_avg_plots, title="Density-Weighted Vertically Averaged wp2", dependant_title="wp2 [m^2/s^2]", panel_type=Panel.TYPE_TIMESERIES)
    ...
    def getThlmSamPlot(self):
        ...
        qi_ncdf = NetCdfVariable('QI', self.sam_file, 1, avging_start_time=sam_start_time, avging_end_time=sam_end_time,timestep=1, fill_zeros=True)
        qi = qi_ncdf.data

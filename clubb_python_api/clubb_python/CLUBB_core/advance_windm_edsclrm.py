...
def advance_windm_edsclrm(
    ...
    fill_holes_type: int, order_xp2_xpyp: int, order_wp2_wp3: int, order_windm: int,
    ...
    result = clubb_f2py.f2py_advance_windm_edsclrm(
        ...
        f_arr(wm_zt), f_arr(kh_zm), f_arr(clubb_params),
        f_arr(ug), f_arr(vg), f_arr(um_ref), f_arr(vm_ref),
        f_arr(wp2), f_arr(up2), f_arr(vp2), f_arr(um_forcing), f_arr(vm_forcing),
        f_arr(edsclrm_forcing), f_arr(p_in_pa),
        f_arr(rho_ds_zm), f_arr(rho_ds_zt), f_arr(invrs_rho_ds_zt),
        ...
        int(fill_holes_type), int(order_xp2_xpyp), int(order_wp2_wp3), int(order_windm),
        np.asarray(upwp_cl_num, dtype=np.int32), np.asarray(vpwp_cl_num, dtype=np.int32),
        f_arr(um), f_arr(vm), f_arr(thlm), f_arr(rtm), f_arr(edsclrm),
        f_arr(upwp), f_arr(vpwp), f_arr(wpedsclrp),

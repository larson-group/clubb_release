  ...
  subroutine xp2_xpyp_lhs( solve_type, dt, l_iter, wp3_on_wp2_zt,       & ! In
    ...
    do k = 2, gr%nz-1, 1
      ...
      if ( ( .not. l_explicit_turbulent_adv_xpyp ) &
         ...
         !        predictive equation (x'^2 or x'y' in this case), y(t) is the
...
!        band_solve
  ...
  subroutine xp2_xpyp_rhs( solve_type, dt, l_iter, a1, a1_zt,     & ! In
    ...
    do k = 2, gr%nz-1, 1
      ...
      if ( l_explicit_turbulent_adv_xpyp ) then
         ...
         rhs(k,1) & 
         = rhs(k,1) &
           + term_ta_explicit_rhs( wpxpyp(kp1), wpxpyp(k), &
                                   rho_ds_zt(kp1), rho_ds_zt(k), &
                                   invrs_rho_ds_zm(k), &
                                   gr%invrs_dzm(k) )
      ...
      * ( - lhs_fnc_output(1) * xapxbp(k) )
      ...
      if ( l_stats_samp ) then
        ...
        if ( l_explicit_turbulent_adv_xpyp ) then
          ...
          call stat_begin_update_pt( ixapxbp_ta, k, &
                   -term_ta_explicit_rhs( wpxpyp(kp1), wpxpyp(k), &
                                          rho_ds_zt(kp1), rho_ds_zt(k), &
                                          invrs_rho_ds_zm(k), &
                                          gr%invrs_dzm(k) ), &
  ...
  pure function term_ta_ADG1_lhs( wp3_on_wp2_ztp1, wp3_on_wp2_zt, &
                                  rho_ds_ztp1, rho_ds_zt, &
                                  invrs_rho_ds_zm, a1_ztp1, a1, &
                                  a1_zt, invrs_dzm, beta, level )  & 
  result( lhs )
    ...
    return
  end function term_ta_ADG1_lhs

  !-----------------------------------------------------------------------------
  pure function term_ta_ADG1_lhs_upwind( a1_zm, a1_zm_p1, a1_zm_m1, &
                                         wp3_on_wp2_p1, wp3_on_wp2, &
                                         wp3_on_wp2_m1, invrs_dzt, &
                                         invrs_dzt_p1, invrs_rho_ds_zm, & 
                                         rho_ds_zm_p1, rho_ds_zm, &
                                         rho_ds_zm_m1, beta ) &
  result( lhs )
  ...
  !-----------------------------------------------------------------------------
    ...
    return
  end function term_ta_ADG1_lhs_upwind

  !=============================================================================
  pure function term_ta_ADG1_rhs( wp2_ztp1, wp2_zt, &
                                  wp3_on_wp2_ztp1, wp3_on_wp2_zt, &
                                  rho_ds_ztp1, rho_ds_zt, invrs_rho_ds_zm, &
                                  a1_ztp1, a1, a1_zt, wpxbp_ztp1, &
                                  wpxbp_zt, wpxap_ztp1, wpxap_zt, &
                                  invrs_dzm, beta ) &
  result( rhs )
    ...
    return
  end function term_ta_ADG1_rhs

  !=============================================================================
  pure function term_ta_explicit_rhs( wpxpypp1, wpxpyp, &
                                      rho_ds_ztp1, rho_ds_zt, &
                                      invrs_rho_ds_zm, &
                                      invrs_dzm ) &
  result( rhs )
    ...
    ! Description:
    ! Turbulent advection of <x'^2> and <x'y'>:  explicit portion of the code.
    !
    ...
    ! pdf_closure_module in pdf_closure_module.F90.  Likewise, the value of
    ! <w'x'y'> is found by integrating over the multivariate PDF of w, x, and y,
    ! as detailed in function calc_wpxpyp_pdf, which is also found in module
    ! pdf_closure_module in pdf_closure_module.F90.
    !
    ! When x and y are the same variable, <x'y'> reduces to <x'^2> and <w'x'y'>
    ! reduces to <w'x'^2>.  The discretization and the code used in this
    ! function will be written generally in terms of <x'y'> and <w'x'y'>, but
    ! also applies to <x'^2> and <w'x'^2>.
    !
  ...
  end function term_ta_explicit_rhs

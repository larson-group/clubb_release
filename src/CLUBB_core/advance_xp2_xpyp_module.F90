...
module advance_xp2_xpyp_module
  ...
  subroutine advance_xp2_xpyp( nzm, nzt, ngrdcol, sclr_dim, sclr_tol, gr, sclr_idx,& ! In
    ...
    Kw2_zm = zt2zm_gpu( nzm, nzt, ngrdcol, gr, Kw2, zero )
    Kw9_zm = zt2zm_gpu( nzm, nzt, ngrdcol, gr, Kw9, zero )
    ...
    if ( iiPDF_type == iiPDF_new_hybrid ) then
      ...
      call xp2_xpyp_lhs( nzm, ngrdcol, dt, & ! In
    ...
    if ( l_min_xp2_from_corr_wx ) then
      ...
      call clip_variance( nzm, ngrdcol, xp2_xpyp_rtp2, dt, threshold_array, & ! In
                          stats_metadata,                                   & ! In  
                          stats_zm,                                         & ! In/out
                          rtp2 )                                              ! In/out
    else
      ...
      call clip_variance( nzm, ngrdcol, xp2_xpyp_rtp2, dt, threshold_array, & ! Intent(in)
                          stats_metadata,                                   & ! In  
                          stats_zm,                                         & ! In/out
                          rtp2 )                                              ! Intent(inout)
    ...
    if ( l_clip_large_rtp2 ) then
      ...
      rtm_zm = zt2zm_gpu( nzm, nzt, ngrdcol, gr, rtm, zero_threshold )
    ...
    if ( l_min_xp2_from_corr_wx ) then
      ...
      call clip_variance( nzm, ngrdcol, xp2_xpyp_thlp2, dt, threshold_array, & ! In
                          stats_metadata,                                   & ! In  
                          stats_zm,                                         & ! In/out
                          thlp2 )                                             ! In/out
      ...
      call clip_variance( nzm, ngrdcol, xp2_xpyp_thlp2, dt, threshold_array, & ! Intent(in)
                          stats_metadata,                                   & ! In  
                          stats_zm,                                         & ! In/out
                          thlp2 )                                             ! Intent(inout)
    ...
    call clip_variance( nzm, ngrdcol, xp2_xpyp_up2, dt, threshold_array, & ! Intent(in)
                        stats_metadata,                                 & ! In  
                        stats_zm,                                       & ! In/out
                        up2 )                                             ! Intent(inout)
    ...
    call clip_variance( nzm, ngrdcol, xp2_xpyp_vp2, dt, threshold_array, & ! Intent(in)
                        stats_metadata,                                 & ! In  
                        stats_zm,                                       & ! In/out
                        vp2 )                                             ! Intent(inout)
    ...
    if ( up2_vp2_sponge_damp_settings%l_sponge_damping ) then
      ...
      do i = 1, ngrdcol
        up2(i,:) = sponge_damp_xp2( nzm, dt, gr%zm(i,:), up2(i,:), w_tol_sqd, &
                                    up2_vp2_sponge_damp_profile )
      end do

      do i = 1, ngrdcol
        vp2(i,:) = sponge_damp_xp2( nzm, dt, gr%zm(i,:), vp2(i,:), w_tol_sqd, &
                                    up2_vp2_sponge_damp_profile )
      end do
    ...
    if ( l_scalar_calc ) then
      ...
      do sclr = 1, sclr_dim, 1
        ...
        call clip_variance( nzm, ngrdcol, clip_sclrp2, dt, threshold_array, & ! Intent(in)
                            stats_metadata,                                 & ! In  
                            stats_zm,                                       & ! In/out
      ...
      do sclr = 1, sclr_dim, 1
        ...
        if  ( sclr == sclr_idx%iisclr_rt ) then
          ...
          call clip_variance( nzm, ngrdcol, clip_sclrprtp, dt, threshold_array, & ! Intent(in)
                              stats_metadata,                                   & ! In  
                              stats_zm,                                         & ! In/out
      ...
      do sclr = 1, sclr_dim, 1
        if ( sclr == sclr_idx%iisclr_thl ) then
          ...
          call clip_variance( nzm, ngrdcol, clip_sclrpthlp, dt, threshold_array, & ! Intent(in)
                              stats_metadata,                                    & ! In  
                              stats_zm,                                          & ! In/out
  ...
  !=============================================================================
  subroutine xp2_xpyp_lhs( nzm, ngrdcol, dt, & ! In
                           lhs_ta, lhs_ma, lhs_diff, lhs_dp1, & ! In
                           lhs ) ! Out
  ...
  subroutine calc_xp2_xpyp_ta_terms( nzm, nzt, ngrdcol, sclr_dim, gr, &
    ...
    a1_coef_zt(:,:) = zm2zt_gpu( nzm, nzt, ngrdcol, gr, a1_coef(:,:), zero_threshold )
    ...
    if ( l_explicit_turbulent_adv_xpyp ) then
      ...
      if ( l_upwind_xpyp_ta ) then
        term_wprtp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wprtp2(:,:) )
      ...
      if ( l_upwind_xpyp_ta ) then
        term_wpthlp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wpthlp2(:,:) )
      ...
      if ( l_upwind_xpyp_ta ) then
        term_wprtpthlp_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wprtpthlp(:,:) )
      ...
      if ( l_scalar_calc ) then
        ...
        do sclr = 1, sclr_dim
          ...
          if ( l_upwind_xpyp_ta ) then
            term_wpsclrp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wpsclrp2(:,:,sclr) )
        ...
        do sclr = 1, sclr_dim
          if ( l_upwind_xpyp_ta ) then
            term_wprtpsclrp_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wpsclrprtp(:,:,sclr) )
        ...
        do sclr = 1, sclr_dim
          if ( l_upwind_xpyp_ta ) then
            term_wpthlpsclrp_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                                       wpsclrpthlp(:,:,sclr) )
      ...
      if ( iiPDF_type == iiPDF_ADG1 ) then  
        ...
        if( .not. l_upwind_xpyp_ta .or. stats_metadata%l_stats_samp ) then
          ...
          wprtp_zt(:,:)  = zm2zt_gpu( nzm, nzt, ngrdcol, gr, wprtp(:,:) )
          wpthlp_zt(:,:) = zm2zt_gpu( nzm, nzt, ngrdcol, gr, wpthlp(:,:) )
        ...
        if ( l_scalar_calc ) then
          ...
          if ( .not. l_upwind_xpyp_ta ) then
            do sclr = 1, sclr_dim
              wpsclrp_zt(:,:,sclr) = zm2zt_gpu( nzm, nzt, ngrdcol, gr, wpsclrp(:,:,sclr) )
            end do
          end if
      ...
      elseif ( iiPDF_type == iiPDF_new ) then
        ...
        ! l_upwind_xpyp_ta is true
        if ( l_upwind_xpyp_ta ) then
          coef_wprtp2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
       ...
       ! l_upwind_xpyp_ta is true
        if ( l_upwind_xpyp_ta ) then
          coef_wpthlp2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
        ...
        ! l_upwind_xpyp_ta is true
        if ( l_upwind_xpyp_ta ) then
          coef_wprtpthlp_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                            pdf_implicit_coefs_terms%coef_wprtpthlp_implicit(:,:) )
          term_wprtpthlp_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
      ...
      elseif ( iiPDF_type == iiPDF_new_hybrid ) then
        ...
        if ( l_upwind_xpyp_ta ) then
          coef_wprtp2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                            pdf_implicit_coefs_terms%coef_wprtp2_implicit(:,:) )
          term_wprtp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
        ...
        if ( l_upwind_xpyp_ta ) then
          coef_wpthlp2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                            pdf_implicit_coefs_terms%coef_wpthlp2_implicit(:,:) )
          term_wpthlp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
        ...
        ! l_upwind_xpyp_ta is true
        if ( l_upwind_xpyp_ta ) then
          coef_wpup2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                               pdf_implicit_coefs_terms%coef_wpup2_implicit(:,:) )
          term_wpup2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
        ...
        ! l_upwind_xpyp_ta is true
        if ( l_upwind_xpyp_ta ) then
          coef_wpvp2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                               pdf_implicit_coefs_terms%coef_wpvp2_implicit(:,:) )
          term_wpvp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
        ...
        if ( l_scalar_calc ) then
          ...
          do sclr = 1, sclr_dim, 1
            ...
            ! l_upwind_xpyp_ta is true
            if ( l_upwind_xpyp_ta ) then
              coef_wpsclrp2_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                        pdf_implicit_coefs_terms%coef_wpsclrp2_implicit(:,:,sclr) )
              term_wpsclrp2_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
            ...
            ! l_upwind_xpyp_ta is true
            if ( l_upwind_xpyp_ta ) then
              coef_wprtpsclrp_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                      pdf_implicit_coefs_terms%coef_wprtpsclrp_implicit(:,:,sclr) )
              term_wprtpsclrp_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                      pdf_implicit_coefs_terms%term_wprtpsclrp_explicit(:,:,sclr) )
              sgn_t_vel_sclrprtp(:,:) &
            ...
            ! l_upwind_xpyp_ta is true
            if ( l_upwind_xpyp_ta ) then
              coef_wpthlpsclrp_implicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                    pdf_implicit_coefs_terms%coef_wpthlpsclrp_implicit(:,:,sclr) )
              term_wpthlpsclrp_explicit_zm(:,:) = zt2zm_gpu( nzm, nzt, ngrdcol, gr, &
                                    pdf_implicit_coefs_terms%term_wpthlpsclrp_explicit(:,:,sclr) )
              sgn_t_vel_sclrpthlp(:,:) &
    ...
    if ( iiPDF_type /= iiPDF_new_hybrid ) then
      ...
      if ( l_upwind_xpyp_ta ) then
        ...
        upwp_zt(:,:) = zm2zt_gpu( nzm, nzt, ngrdcol, gr, upwp(:,:) )
        vpwp_zt(:,:) = zm2zt_gpu( nzm, nzt, ngrdcol, gr, vpwp(:,:) )
  ...
  !=============================================================================
  subroutine term_dp1_lhs( nzm, ngrdcol, &
                           Cn, invrs_tau_zm, &
                           lhs )
    ! Description:
  ...
  subroutine update_xp2_mc( gr, nzm, nzt, ngrdcol, dt, cloud_frac, rcm, rvm, thlm, &
    ...
    rtp2_mc = zt2zm_gpu( nzm, nzt, ngrdcol, gr, rtp2_mc_zt )
    ...
    thlp2_mc = zt2zm_gpu( nzm, nzt, ngrdcol, gr, thlp2_mc_zt )
    ...
    wprtp_mc    = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wprtp_mc_zt )
    wpthlp_mc   = zt2zm_gpu( nzm, nzt, ngrdcol, gr, wpthlp_mc_zt )
    rtpthlp_mc  = zt2zm_gpu( nzm, nzt, ngrdcol, gr, rtpthlp_mc_zt )

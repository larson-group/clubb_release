    ...
    do i=1,zt%nn
      ...
      case ('eff_rad_cloud')
        ...
        call stat_assign( ieff_rad_cloud, "eff_rad_cloud", & 
             "Cloud drop effective volume radius (microns)", "micron", zt )
        k = k + 1
      ...
      case ('eff_rad_ice')
        ...
        call stat_assign( ieff_rad_ice, "eff_rad_ice", & 
             "Ice effective volume radius (microns)", "micron", zt )
        k = k + 1

      case ('eff_rad_snow')
        ieff_rad_snow = k
        call stat_assign( ieff_rad_snow, "eff_rad_snow", & 
             "Snow effective volume radius (microns)", "micron", zt )
        k = k + 1

      case ('eff_rad_rain')
        ieff_rad_rain = k
        call stat_assign( ieff_rad_rain, "eff_rad_rain", & 
             "Rain drop effective volume radius (microns)", "micron", zt )
        k = k + 1

      case ('eff_rad_graupel')
        ieff_rad_graupel = k
        call stat_assign( ieff_rad_graupel, "eff_rad_graupel", & 
             "Graupel effective volume radius (microns)", "micron", zt )
        k = k + 1
     ...
     case ( 'AKm_rcm' )
      ...
      case ('rtm_mfl')
        ...
        call stat_assign( irtm_mfl, "rtm_mfl", &
              "rtm correction due to monotonic flux limiter", "kg/kg/s", zt)
        k = k + 1
      ...
      case ('thlm_mfl')
        ...
        call stat_assign( ithlm_mfl, "thlm_mfl", &
              "thlm correction due to monotonic flux limiter", "K/s", zt)
        k = k + 1
      ...
      case ('thlm_tacl')
        ...
        call stat_assign( ithlm_tacl, "thlm_tacl", & 
              "thlm correction due to t.a. term (wpthlp) clipping", "K/s", zt)
        k = k + 1
      ...
      case ( 'LH_Ncp2_zt' )
        ...
        call stat_assign( iLH_Ncp2_zt, "LH_Ncp2_zt", & 
             "Variance of the latin hypercube estimate of Nc", "count^2/kg^2", zt )
        k = k + 1

      case ( 'LH_Nrp2_zt' )
        iLH_Nrp2_zt = k
        call stat_assign( iLH_Nrp2_zt, "LH_Nrp2_zt", & 
             "Variance of the latin hypercube estimate of Nr", "count^2/kg^2", zt )
        k = k + 1
      ...
      case ( 'LH_rrainp2_zt' )
        ...
        call stat_assign( iLH_rrainp2_zt, "LH_rrainp2_zt", & 
             "Variance of the latin hypercube estimate of rrain", "kg^2/kg^2", zt )
        k = k + 1

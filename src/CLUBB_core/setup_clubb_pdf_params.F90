...
!        stat_update_var,    & ! Procedure(s)
       ...
       do k = 1, nzt, 1
         do i = 1, ngrdcol
            ...
            if ( hydromet(i,k,j) >= hm_metadata%hydromet_tol(j) ) then
              ...
              hydrometp2_zt(i,k,j) &
              = ( ( hm_metadata%hmp2_ip_on_hmm2_ip(j) + one ) / precip_fracs%precip_frac(i,k) - one ) &
                * hydromet(i,k,j)**2
            else
    ...
    if ( l_diagnose_correlations ) then
      ...
      if ( l_fix_w_chi_eta_correlations &
        ...
        call calc_corr_norm_and_cholesky_factor( nzt, ngrdcol, iiPDF_type,                  & ! intent(in)
                                                 pdf_dim, hm_metadata,                      & ! intent(in)
        ...
        do i = 1, ngrdcol
          rtp2_zt_from_chi(i,:) &
            = compute_rtp2_from_chi( pdf_params%stdev_chi_1(i,:), pdf_params%stdev_chi_2(i,:), &
                                     ...
                                     pdf_params%mixt_frac(i,:),                                &   
                                     corr_array_1_n(i,:,hm_metadata%iiPDF_chi,hm_metadata%iiPDF_eta),                  &

!-----------------------------------------------------------------------
! $Id: semiimplicit_clip.F90,v 1.4 2008-08-08 15:42:17 griffinb Exp $
!===============================================================================
...
! threshold value for variable f.
!
...
!
! where dt_clip is the time scale for the clipping term.  The difference between
! the threshold value and f_unclipped is defined as f_diff:
!
...
! clipping term must be linearized.  A Taylor Series expansion (truncated after 
! the first derivative term) of R(f_diff) around f_diff = f_diff(t) is used to 
! linearize the term.  However, the Heaviside Step function, H(f_diff), is not 
! differentiable when f_diff(t) = 0, as the function jumps at that point.  
! Likewise, the function R(f_diff) is not differentiable when f_diff(t) = 0, as 
! the function has a corner at that point.  Therefore, a new function, 
! F_R(f_diff) is used as an approximation of R(f_diff).  Function F_R(f_diff) is
! a three-piece function that has the exact same value as R(f_diff) when 
! f_diff <= -sigma or f_diff >= sigma (sigma is an arbitrarily declared value). 
! However, when -sigma < f_diff < sigma, a parabolic function is used to 
! approximate the corner found in R(f_diff).  The parabolic function needs to 
! have the same values at f_diff = -sigma and f_diff = sigma as does R(f_diff). 
! Furthermore, the derivative of the parabolic function (with respect to f_diff)
! needs to have the same values at f_diff = -sigma and f_diff = sigma as does 
! d(R)/d(f_diff).  The parabolic function that satisfies these properities is:  
...
!       
! The approximation is substituted into the (df/dt)_clipping equation.  The rate
! of change of variable f due to clipping with the upper threshold is:
!
! (df/dt)_clipping
! = + (1/dt_clip)
...
! advanced to in solving the d(f)/dt equation.
!
!
...
! dt_clip coefficient:  A coefficient with dimensionless units that must have a 
!                       value greater than 0.  A value of 1 will set the 
...
!        to a "+".
!
! The timestep index (t+1) means that the value of f being used is from the next
! timestep, which is being advanced to in solving the d(f)/dt equation.
!
! While the formulas are the same for both the upper threshold and the lower 
! threshold, the values of A_fnc, B_fnc, and f_diff will differ between the two
! thresholds.
!
! The overall implicit (LHS) portion for the clipping term is the sum of the 
! implicit portion from the upper threshold and the implicit portion from the 
! lower threshold.
...
if ( lupper_thresh ) then
   ...
   ! f_diff is the difference between the threshold value and f_unclipped.  In 
   ! regards to the upper threshold, it is defined as 
   ! upper_threshold - f_unclipped.
   f_diff = upper_threshold - f_unclipped

   ! Compute the values of functions A_fnc and B_fnc evaluated at f_diff(t) for 
   ! the upper threshold.
...
if ( llower_thresh ) then
   ...
   ! f_diff is the difference between the threshold value and f_unclipped.  In 
   ! regards to the lower threshold, it is defined as 
...
!
! The values of A_fnc, B_fnc, and f_diff will differ between the two thresholds.
!
! The overall explicit (RHS) portion for the clipping term is the sum of the 
! explicit portion from the upper threshold and the explicit portion from the 
! lower threshold.
...
! clipping.  Both of the functions are based on the values of f_diff(t) and the 
! parameter sigma.  One function is A_fnc, which is F_R(f_diff) evaluated at 
! f_diff = f_diff(t).  F_R(f_diff) is a three-piece function that is used to 
! approximate function R(f_diff).  The other function is B_fnc, the derivative 
! with respect to f_diff of function A_fnc.  In other words, B_fnc is 
! ( d F_R / d f_diff ) evaluated at f_diff = f_diff(t).
!
...
! Note:  The constant eps is there in case only one threshold applies, and it 
! has a value of 0 (or very close to 0).  However, eps is a very small number, 
! and therefore it will not start curbing values until they get extremely
! close to the threshold.  A larger constant value may work better.
...
! R(f_diff(t)) = { f_diff(t) * [ 1 - H(f_diff(t)) ] }.  This is needed because 
! the R(f_diff(t)) is not differentiable at point f_diff(t) = 0, as the function
! has a corner at that point.  Function A_fnc is differentiable at all points.  
! It is evaluated for f_diff at timestep index (t).

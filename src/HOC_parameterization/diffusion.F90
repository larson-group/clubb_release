...
! both implicit and explicit components, such that:
!
...
! advanced to in solving the d(var_zt)/dt equation.
!
! The implicit portion of this term is discretized as follows:
!
! The values of var_zt are found on the thermodynamic levels, while the values 
! of K_zm are found on the momentum levels.  The derivatives (d/dz) of var_zt 
! are taken over the intermediate momentum levels.  At the intermediate momentum
! levels, d(var_zt)/dz is multiplied by ( K_zm + nu ).  Then, the derivative of 
...
!        function will have to be multiplied by (1/2).  For a Crank-Nicholson 
!        scheme, the right-hand side (explicit) component needs to be computed 
!        by multiplying the left-hand side results by (1/2), reversing the sign
!        on each left-hand side element, and then multiplying each element by 
!        the appropriate var_zt(t) value from the appropriate vertical level.
!
!
! Boundary Conditions:
!
! 1) Zero-flux boundary conditions.
!    This function is set up to use a zero-flux boundary condition at both the 
!    lower boundary level (k=1) and the upper boundary level (k=gr%nnzp).  This 
!    means that the derivative of var_zt, d(var_zt)/dz, equals 0 at both the 
!    lower boundary and the upper boundary.
!
!    In order to discretize the lower boundary condition, consider a new level 
!    outside the model (thermodynamic level 0) just below the lower boundary 
!    level (thermodynamic level 1).  The value of var_zt at the level just 
!    outside the model is defined to be the same as the value of var_zt at the 
!    lower boundary level.  Therefore, the value of d(var_zt)/dz between the 
!    level just outside the model and the lower boundary level is 0, satisfying 
!    the zero-flux boundary condition.  The other value for d(var_zt)/dz 
!    (between thermodynamic level 2 and thermodynamic level 1) is taken over the
!    intermediate momentum level (momentum level 1), where it is multiplied by 
!    the factor ( K_zm(1) + nu ).  Then, the derivative of the whole expression
...
!
!    The result is dependent only on values of K_zm found at momentum level 1 
!    and values of var_zt found at thermodynamic levels 1 and 2.  Thus, it only 
!    affects 2 diagonals on the left-hand side matrix.
!
!    The same method can be used to discretize the upper boundary by considering
!    a new level outside the model just above the upper boundary level.
!
! 2) Fixed-point boundary conditions.
!    Many equations in the model use fixed-point boundary conditions rather than
!    zero-flux boundary conditions.  This means that the value of var_zt stays 
!    the same over the course of the timestep at the lower boundary, as well as
!    at the upper boundary.
!
!    In order to discretize the boundary conditions for equations requiring 
!    fixed-point boundary conditions, either:
!    a) in the parent subroutine or function (that calls this function), loop 
!       over all vertical levels from the second-lowest to the second-highest, 
!       ignoring the boundary levels.  Then set the values at the boundary 
!       levels in the parent subroutine; or
!    b) in the parent subroutine or function, loop over all vertical levels and 
!       then overwrite the results at the boundary levels.
!
!    Either way, at the boundary levels, an array with a value of 1 at the main
!    diagonal on the left-hand side and with values of 0 at all other diagonals
!    on the left-hand side will preserve the right-hand side value at that 
!    level, thus satisfying the fixed-point boundary conditions.
!
!
! Conservation Properties:
!
! When zero-flux boundary conditions are used, this technique of discretizing 
! the eddy diffusion term leads to conservative differencing.  When conservative
! differencing is in place, the column totals for each column in the left-hand 
! side matrix (for the eddy diffusion term) should be equal to 0.  This ensures 
! that the total amount of the quantity var_zt over the entire vertical domain 
! is being conserved, meaning that nothing is lost due to diffusional effects.
!
! To see that this conservation law is satisfied, compute the eddy diffusion of 
! var_zt and integrate vertically.  In discretized matrix notation (where "i" 
! stands for the matrix column and "j" stands for the matrix row):
!
!  0 = Sum_j Sum_i ( 1/dzt )_i ( dzt * ((K_zm+nu)*dzm) )_ij (var_zt)_j.
!
! The left-hand side matrix, ( dzt * ((K_zm+nu)*dzm) )_ij, is partially written 
! below.  The sum over i in the above equation removes dzt everywhere from the 
! matrix below.  The sum over j leaves the column totals that are desired.
!
...
!
! Note:  The matrix shown is a tridiagonal matrix.  For a band diagonal matrix 
!        (with 5 diagonals), there would be an extra row between each of the 
!        rows shown and an extra column between each of the columns shown.  
!        However, for the purposes of the var_zt eddy diffusion term, those
!        extra row and column values are all 0, and the conservation properties 
!        of the matrix aren't effected.
!              
! If fixed-point boundary conditions are used, the matrix entries at 
! level 1 (k=1) read:  1   0   0; which means that conservative differencing is
! not in play.  The total amount of var_zt over the entire vertical domain is 
! not being conserved, as amounts of var_zt may be fluxed out through the upper 
! boundary or lower boundary through the effects of diffusion.
!
...
! Timestep index (t) stands for the index of the current timestep, while 
! timestep index (t+1) stands for the index of the next timestep, which is being
! advanced to in solving the d(var_zm)/dt equation.
!
! The implicit portion of this term is discretized as follows:
!
! The values of var_zm are found on the momentum levels, while the values of 
! K_zt are found on the thermodynamic levels.  The derivatives (d/dz) of var_zm 
! are taken over the intermediate thermodynamic levels.  At the intermediate 
! thermodynamic levels, d(var_zm)/dz is multiplied by ( K_zt + nu ).  Then, the 
! derivative of the whole mathematical expression is taken over the central
...
!        by multiplying the left-hand side results by (1/2), reversing the sign
!        on each left-hand side element, and then multiplying each element by 
!        the appropriate var_zm(t) value from the appropriate vertical level.
!
!
! Boundary Conditions:
!
...
!    This function is set up to use a zero-flux boundary condition at both the 
!    lower boundary level (k=1) and the upper boundary level (k=gr%nnzp).  This
!    means that the derivative of var_zm, d(var_zm)/dz, equals 0 at both the 
!    lower boundary and the upper boundary.
!
!    In order to discretize the lower boundary condition, consider a new level 
!    outside the model (momentum level 0) just below the lower boundary level 
!    (momentum level 1).  The value of var_zm at the level just outside the 
!    model is defined to be the same as the value of var_zm at the lower 
!    boundary level.  Therefore, the value of d(var_zm)/dz between the level
!    just outside the model and the lower boundary level is 0, satisfying the 
!    zero-flux boundary condition.  The other value for d(var_zm)/dz (between 
!    momentum level 2 and momentum level 1) is taken over the intermediate 
!    thermodynamic level (thermodynamic level 2), where it is multiplied by the 
!    factor ( K_zt(2) + nu ).  Then, the derivative of the whole expression is 
...
!    level 2 and values of var_zm found at momentum levels 1 and 2.  Thus, it 
!    only affects 2 diagonals on the left-hand side matrix.
!
!    The same method can be used to discretize the upper boundary by considering
!    a new level outside the model just above the upper boundary level.
!
! 2) Fixed-point boundary conditions.
!    Many equations in the model use fixed-point boundary conditions rather than
!    zero-flux boundary conditions.  This means that the value of var_zm stays 
!    the same over the course of the timestep at the lower boundary, as well as 
!    at the upper boundary.
!
!    In order to discretize the boundary conditions for equations requiring 
...
! differencing is in place, the column totals for each column in the left-hand 
! side matrix (for the eddy diffusion term) should be equal to 0.  This ensures 
! that the total amount of the quantity var_zm over the entire vertical domain 
! is being conserved, meaning that nothing is lost due to diffusional effects.
!
! To see that this conservation law is satisfied, compute the eddy diffusion of 
! var_zm and integrate vertically.  In discretized matrix notation (where "i" 
! stands for the matrix column and "j" stands for the matrix row):
!
!  0 = Sum_j Sum_i ( 1/dzm )_i ( dzm * ((K_zt+nu)*dzt) )_ij (var_zm)_j.
!
! The left-hand side matrix, ( dzm * ((K_zt+nu)*dzt) )_ij, is partially written 
! below.  The sum over i in the above equation removes dzm everywhere from the 
! matrix below.  The sum over j leaves the column totals that are desired.
!
! Left-hand side matrix contributions from eddy diffusion term; first four 
! vertical levels:
...
!        (with 5 diagonals), there would be an extra row between each of the 
!        rows shown and an extra column between each of the columns shown.  
!        However, for the purposes of the var_zm eddy diffusion term, those 
!        extra row and column values are all 0, and the conservation properties
!        of the matrix aren't effected.
!              
! If fixed-point boundary conditions are used, the matrix entries at 
! level 1 (k=1) read:  1   0   0; which means that conservative differencing is 
! not in play.  The total amount of var_zm over the entire vertical domain is 
! not being conserved, as amounts of var_zm may be fluxed out through the upper
! boundary or lower boundary through the effects of diffusion.
!
! Brian Griffin.  April 26, 2008.

...
#ifdef CLUBB
   use clubb_api_module, only: fstderr, Lv, Ls, Cp, Rv, Rd, T_freeze_K, rho_lw, grav, EP_2 => ep
#else
...
!..Create bins of graupel (from min diameter up to 5 cm).
      ...
      call radar_init
#if !defined( CLUBB ) && !defined( UWM_MISC )
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      SUBROUTINE M2005MICRO_GRAUPEL(QC3DTEN,QI3DTEN,QNI3DTEN,QR3DTEN,NC3DTEN,&
                        ...
                        NIM_MORR_CL, &
                        QC_INST, QR_INST, QI_INST, QS_INST, QG_INST, &
...
! HM ADDED GRAUPEL PROCESSES
     ...
     REAL, DIMENSION(KMS:KME) ::  NACT      ! Number of cloud drops formed by activated aerosols 
     REAL, DIMENSION(KMS:KME) ::  SIZEFIX_NR  ! Adjustment to rain drop number concentration for large/small drops
     REAL, DIMENSION(KMS:KME) ::  SIZEFIX_NC  ! Adjustment to cloud drop number concentration for large/small drops
     REAL, DIMENSION(KMS:KME) ::  SIZEFIX_NI  ! Adjustment to ice number concentration for large/small drops
     REAL, DIMENSION(KMS:KME) ::  SIZEFIX_NS  ! Adjustment to snow number concentration for large/small drops
     REAL, DIMENSION(KMS:KME) ::  SIZEFIX_NG  ! Adjustment to graupel number concentration for large/small drops
     REAL, DIMENSION(KMS:KME) ::  NEGFIX_NI    ! Removal of negative ice number concentration 
     REAL, DIMENSION(KMS:KME) ::  NEGFIX_NS    ! Removal of negative snow number concentration 
     REAL, DIMENSION(KMS:KME) ::  NEGFIX_NC    ! Removal of negative cloud drop number concentration 
     REAL, DIMENSION(KMS:KME) ::  NEGFIX_NR    ! Removal of negative rain drop number concentration 
     REAL, DIMENSION(KMS:KME) ::  NEGFIX_NG    ! Removal of negative graupel number concentration 
     REAL, DIMENSION(KMS:KME) ::  NIM_MORR_CL  ! Clipping of large ice concentrations
     REAL, DIMENSION(KMS:KME) ::  QC_INST      ! Change in cloud mixing ratio from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  QR_INST      ! Change in rain mixing ratio from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  QI_INST      ! Change in ice mixing ratio from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  QS_INST      ! Change in snow mixing ratio from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  QG_INST      ! Change in graupel mixing ratio from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  NC_INST      ! Change in cloud number concentration from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  NR_INST      ! Change in rain number concentration from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  NI_INST      ! Change in ice number concentration from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  NS_INST      ! Change in snow number concentration from instantaneous processes
     REAL, DIMENSION(KMS:KME) ::  NG_INST      ! Change in graupel number concentration from instantaneous processes
...
! Initialize rain sedimentation velocity variable (rstorer)
! When using Sun Studio compiler, uninitialized variables are
! set to NAN. Since FR is output, we need to initialize this
! variable in order to prevent a floating point error.
! Discussed in mmf:ticket:18:comment:6
...
! MELT VERY SMALL SNOW AND GRAUPEL MIXING RATIOS, ADD TO RAIN
       IF (QNI3D(K).LT.1.E-6) THEN
           QR_INST(K) = QR_INST(K) + QNI3D(K)/DT
           QS_INST(K) = QS_INST(K) - QNI3D(K)/DT
           NR_INST(K) = NR_INST(K) + NS3D(K)/DT
           NS_INST(K) = NS_INST(K) - NS3D(K)/DT
       ...
       IF (QG3D(K).LT.1.E-6) THEN
           QR_INST(K) = QR_INST(K) + QG3D(K)/DT
           QG_INST(K) = QG_INST(K) - QG3D(K)/DT
           NR_INST(K) = NR_INST(K) + NG3D(K)/DT
           NG_INST(K) = NG_INST(K) - NG3D(K)/DT
...
! MAKE SURE NUMBER CONCENTRATIONS AREN'T NEGATIVE
      ...
      if (NI3D(K) .lt. 0.) then
        NEGFIX_NI(K) = NEGFIX_NI(K) + NI3D(K)/DT
      end if
      if (NS3D(K) .lt. 0.) then
        NEGFIX_NS(K) = NEGFIX_NS(K) + NS3D(K)/DT
      end if
      if (NC3D(K) .lt. 0.) then
        NEGFIX_NC(K) = NEGFIX_NC(K) + NC3D(K)/DT
      end if
      if (NR3D(K) .lt. 0.) then
        NEGFIX_NR(K) = NEGFIX_NR(K) + NR3D(K)/DT
      end if
      if (NG3D(K) .lt. 0.) then
        NEGFIX_NG(K) = NEGFIX_NG(K) + NG3D(K)/DT
      end if
...
! ADJUST VARS
        ...
        IF (LAMR(K).LT.LAMMINR) THEN
          ...
          SIZEFIX_NR(K) = SIZEFIX_NR(K) + ( NR3D(K) - TMPNUM) / DT
...
! LAMMAX, 1 MICRON
        ...
        IF (LAMC(K).LT.LAMMIN) THEN
          ...
          SIZEFIX_NC(K) = SIZEFIX_NC(K) + ( NC3D(K) - TMPNUM) / DT
...
! ADJUST VARS
        ...
        IF (LAMS(K).LT.LAMMINS) THEN
          ...
          TMPNUM = NS3D(K)
          NS3D(K) = N0S(K)/LAMS(K)
          SIZEFIX_NS(K) = SIZEFIX_NS(K) + ( NS3D(K) - TMPNUM) / DT
...
! ADJUST VARS
        ...
        IF (LAMG(K).LT.LAMMING) THEN
          ...
          TMPNUM = NG3D(K)
          NG3D(K) = N0G(K)/LAMG(K)
          SIZEFIX_NG(K) = SIZEFIX_NG(K) + ( NG3D(K) - TMPNUM) / DT
...
#ifdef CLUBB
        !Checking to be sure that rain evaporation is not occurring within cloud
        IF (PRE(K) .LT. 0.0 .AND. CF3D(K) .GT. cloud_frac_thresh) THEN
          write(fstderr,*) "Rain evaporation occurring within cloud"
!          stop "Error in Morrison microphysics."
        END IF

        !Checking to be sure that PRACS and PRACG are not included in the tendency
        !These terms are treated with the opposite sign later, and if both
        !occurrences are included, the budgets of rrainm, rsnowm, and rgraupelm
        !will not balance.
        IF (PRACS(K) .ne. 0. .or. PRACG(K) .ne. 0.) THEN
          write(fstderr,*) "PRACS or PRACG is violating expectation of zero value"
          stop "Error in Morrison microphysics."
        END IF
#endif
...
! ADJUST VARS
        ...
        IF (LAMI(K).LT.LAMMINI) THEN
          ...
          NI3D(K) = N0I(K)/LAMI(K)
          SIZEFIX_NI(K) = SIZEFIX_NI(K) + ( NI3D(K) - TMPNUM) / DT          
...
#ifdef CLUBB
        !Checking to be sure that PCC is not included in the tendency twice
        IF (PCC(K) .ne. 0.) THEN
          write(fstderr,*) "PCC has a nonzero value and will be added to QC3DTEN twice."
          stop "Error in Morrison microphysics."
        END IF
#endif
...
!    Rain drop number process rates
          ...
          NSMLTS(K) = NSMLTS(K) * CF3D(K)
          NSAGG(K) = NSAGG(K) * CF3D(K)
          NPRCI(K) = NPRCI(K) * CF3D(K)
          NSCNG(K) = NSCNG(K) * CF3D(K)
          NSUBS(K) = NSUBS(K) * CF3D(K)

          PCC(K) = PCC(K) * CF3D(K)
          NNUCCC(K) = NNUCCC(K) * CF3D(K)
          NPSACWS(K) = NPSACWS(K) * CF3D(K)         
          NPRA(K) = NPRA(K) * CF3D(K)
          NPRC(K) = NPRC(K) * CF3D(K)
          NPSACWI(K) = NPSACWI(K) * CF3D(K)
          NPSACWG(K) = NPSACWG(K) * CF3D(K)
          NPRAI(K) = NPRAI(K) * CF3D(K)
          NMULTS(K) = NMULTS(K) * CF3D(K) 
          NMULTG(K) = NMULTG(K) * CF3D(K)
          NMULTR(K) = NMULTR(K) * CF3D(K)
          NMULTRG(K) = NMULTRG(K) * CF3D(K)
          NNUCCD(K) = NNUCCD(K) * CF3D(K)
          NSUBI(K) = NSUBI(K) * CF3D(K)
          NGMLTG(K) = NGMLTG(K) * CF3D(K)
          NSUBG(K) = NSUBG(K) * CF3D(K)
          NACT(K) = NACT(K) * CF3D(K)
          NEGFIX_NI(K) = NEGFIX_NI(K) * CF3D(K)
          NEGFIX_NS(K) = NEGFIX_NS(K) * CF3D(K)
          NEGFIX_NC(K) = NEGFIX_NC(K) * CF3D(K)
          NEGFIX_NR(K) = NEGFIX_NR(K) * CF3D(K)
          NEGFIX_NG(K) = NEGFIX_NG(K) * CF3D(K)
          SIZEFIX_NR(K) = SIZEFIX_NR(K) * CF3D(K)
          SIZEFIX_NC(K) = SIZEFIX_NC(K) * CF3D(K)
          SIZEFIX_NI(K) = SIZEFIX_NI(K) * CF3D(K)
          SIZEFIX_NS(K) = SIZEFIX_NS(K) * CF3D(K)
          SIZEFIX_NG(K) = SIZEFIX_NG(K) * CF3D(K)
          QC_INST(K) = QC_INST(K) * CF3D(K) 
          QR_INST(K) = QR_INST(K) * CF3D(K) 
          QI_INST(K) = QI_INST(K) * CF3D(K)
          QS_INST(K) = QS_INST(K) * CF3D(K)
          QG_INST(K) = QG_INST(K) * CF3D(K)
          NC_INST(K) = NC_INST(K) * CF3D(K)
          NR_INST(K) = NR_INST(K) * CF3D(K)
          NI_INST(K) = NI_INST(K) * CF3D(K)
          NS_INST(K) = NS_INST(K) * CF3D(K)
          NG_INST(K) = NG_INST(K) * CF3D(K)
...
! rstorer - 11/12/13
! Commenting out these lines of code to produce better snow number
! concentrations. This is described in clubb:comment:34:ticket:603
#ifndef CLUBB
...
! ADD MELTING OF CLOUD ICE TO FORM RAIN
        ...
        IF (QI3D(K).GE.QSMALL.AND.T3D(K).GE.TMELT) THEN
           QR_INST(K) = QR_INST(K) + QI3D(K)/DT
           QI_INST(K) = QI_INST(K) - QI3D(K)/DT
           NR_INST(K) = NR_INST(K) + NI3D(K)/DT
           NI_INST(K) = NI_INST(K) - NI3D(K)/DT
...
! HOMOGENEOUS FREEZING OF CLOUD WATER
        ...
        IF (T3D(K).LE.233.15.AND.QC3D(K).GE.QSMALL) THEN
           QI_INST(K) = QI_INST(K) + QC3D(K)/DT
           QC_INST(K) = QC_INST(K) - QC3D(K)/DT
           NI_INST(K) = NI_INST(K) + NC3D(K)/DT
           NC_INST(K) = NC_INST(K) - NC3D(K)/DT
...
! HOMOGENEOUS FREEZING OF RAIN
        ...
        IF (T3D(K).LE.233.15.AND.QR3D(K).GE.QSMALL) THEN
           QG_INST(K) = QG_INST(K) + QR3D(K)/DT
           QR_INST(K) = QR_INST(K) - QR3D(K)/DT
           NG_INST(K) = NG_INST(K) + NR3D(K)/DT
           NR_INST(K) = NR_INST(K) - NR3D(K)/DT
        ...
        IF (T3D(K).LE.233.15.AND.QR3D(K).GE.QSMALL) THEN
           QS_INST(K) = QS_INST(K) + QR3D(K)/DT
           QR_INST(K) = QR_INST(K) - QR3D(K)/DT
           NS_INST(K) = NS_INST(K) + NR3D(K)/DT
           NR_INST(K) = NR_INST(K) - NR3D(K)/DT
...
! OF DROPLETS, ESPECIALLY WHEN INUM = 1, SET MAX AT 10 CM-3
          IF (NI3D(K) .GT. (10.E6/RHO(K))) THEN
              NIM_MORR_CL(K) = (NI3D(K) - (10.E6/RHO(K)))/DT
          END IF
...
!   None
!-------------------------------------------------------------------------------
    use clubb_api_module, only: Lv, Ls, Cp ! Constant(s)

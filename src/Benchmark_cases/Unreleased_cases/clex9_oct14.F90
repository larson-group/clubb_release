...
!       Description:
!       Contains subroutines for the CLEX-9: Oct. 14 case.
...
!----------------------------------------------------------------------
  ...
  use constants_clubb, only: Cp, Lv, zero_threshold, fstderr ! Variable(s)

  use parameters_model, only: sclr_dim, edsclr_dim ! Variables
...
!            use the "cos_solar_zen" function for this case.
 xi_abs = real( cos_solar_zen( 14, 10, 2001, time, rlat, rlon ) )
...
!-----------------------------------------------------------------------
  ...
  ! Enter the final and rtm tendency

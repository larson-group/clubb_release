...
#define SCLR_RT 2
!----------------------------------------------------------------------
! $Id: arm_3year.F90,v 1.4 2008-07-31 19:34:16 faschinj Exp $
module arm_3year
...
public :: arm_3year_tndcy, arm_3year_sfclyr, arm_3year_init
...
!----------------------------------------------------------------------
subroutine arm_3year_tndcy( time, wm_zt, wm_zm,  & 
                         thlm_forcing, rtm_forcing,  & 
                         um_hoc_grid, vm_hoc_grid, & 
                         sclrm_forcing )
!       Description:
!       Subroutine to set thetal and total water tendencies for ARM 3 year case
...
if( time_frac == -1.0 ) then
        call clubb_debug & 
           (1,"times not sorted in arm_3year_tndcy")
endif
...
thlm_forcing(1:gr%nnzp) = zlinterp_fnc & 
         ( gr%nnzp, nz, gr%zt(:), z, thlm_t_interp )


! Vince Larson added in poor man's radiation. 
!     Do not use at the same time as BugsRad.  15 Dec 2007
!thlm_forcing(2:50) = thlm_forcing(2:50) - 5.0e-6
...
return
end subroutine arm_3year_tndcy


!----------------------------------------------------------------------
subroutine arm_3year_sfclyr & 
           ( time, z, rho0, & 
...
! Reading files located in /model/arm_3year_forcings/

        ...
        for line in srcFile:
            ...
            if    re.search(r"\breal\b", line, re.IGNORECASE) is None \
                ...
                if "&" in line: # then line continues
                    ...
                    parts = []
                    isPartOfDecLine = False
        if parts: yield ''.join(parts) # return any declarations at end
...
def findThreadPrivates(fileName):
    ...
    with open(fileName) as srcFile:
        ...
        listOfThreadPrivates = [re.sub(r"(?i)&\s*!\$omp","",x) for x in listOfThreadPrivates] # Remove openmp directives
...
def main():
    ...
    clubbFileNames = findFiles("/home/vlarson/cam_issue_121/src/CLUBB_core/")
    silhsFileNames = findFiles("/home/vlarson/cam_issue_121/src/SILHS/")
    ...
    #outputFile.close()
    print("passedTest = %s" %passedTest)
    print("failedFiles = %s" %str(failedFiles))
    return passedTest

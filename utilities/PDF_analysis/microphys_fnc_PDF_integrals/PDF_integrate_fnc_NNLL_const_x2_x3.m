% $Id$
function PDF_integrate_fnc_NNLL_const_x2_x3( a_exp, b_exp, x2_opt )
...
%
% In this special case, the x2 and x3 variables are held constant.
...
num_partitions_x1 = 500;
num_partitions_x4 = 1000;

fprintf( 'number of partitions: x1: %d; x4: %d\n', ...
         num_partitions_x1, num_partitions_x4 );

% Partition size for x1 and x4.
delta_x1 = ( integral_ub_x1 - integral_lb_x1 ) / num_partitions_x1;
delta_x4 = ( integral_ub_x4 - integral_lb_x4 ) / num_partitions_x4;

fprintf( 'partition sizes: x1: %7.5e; x4: %7.5e\n', delta_x1, delta_x4 );
...
for q = 0:1:b_exp
   ...
   for iter_x1 = 1:1:num_partitions_x1
      for iter_x4 = 1:1:num_partitions_x4
         ...
         sum = sum ...
               + delta_x1 * delta_x4 ...
                 * ( x1 - x1_bar )^a_exp * x4^(gamma_exp*q) ...
                 * PDF_comp_bivar_NL( x1, x4, mu_x1, mu_x4_n, ...
...
if ( ( a_exp == 1 ) && ( b_exp == 1 ) )
   ...
   if ( mu_x2 <= 0.0 )
      ...
      = mu_x2^alpha_exp * mu_x3^beta_exp ...
        ...
        * exp( mu_x4_n * gamma_exp + 0.5 * sigma_x4_n^2 * gamma_exp^2 ) ...

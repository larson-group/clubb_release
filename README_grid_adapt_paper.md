...
1. Run `./run_scripts/run_scm.bash arm`, `./run_scripts/run_scm.bash astex_a209`, `./run_scripts/run_scm.bash dycoms2_rf01` and `./run_scripts/run_scm.bash gabls2`
...
2. Run `./run_scripts/run_scm.bash arm`, `./run_scripts/run_scm.bash astex_a209`, `./run_scripts/run_scm.bash dycoms2_rf01` and `./run_scripts/run_scm.bash gabls2`
...
```
! $Id$
! Parameter file for DYCOMS II RF01 Sc case

&model_setting

! Model Settings

runtype = "dycoms2_rf01" ! Case name.

! For grid_type = 1 nzmax can be any nunber
!nzmax = 132       ! Number of vertical levels.
nzmax = 426       ! Number of vertical levels.
!nzmax = 73       ! Number of vertical levels.
...
deltaz_nl  = 20.0    ! Distance between grid levels on evenly-spaced grid.      [m]
zm_init_nl = 0.0 ! Minimum Altitude of lowest momentum level on any grid. [m]
!zm_top_nl = 5000.0 ! Maximum Altitude of highest momentum level on any grid. [m]
zm_top_nl = 8500.0 ! Maximum Altitude of highest momentum level on any grid. [m]
...
day   = 10    ! Day of model start (1 to 31).
...
time_initial = 21600.0  ! Model start time [seconds since midnight UTC on start date]
time_final =  36000.0   ! Model end time [seconds since midnight UTC on start date]
...
p_sfc_nl = 1017.e2   ! Pressure at model base              [Pa]
...
l_stats       = .true.         ! Flag for statistical output.
fname_prefix = "dycoms2_rf01"  ! Prefix of output filename.
stats_tsamp  = 60.          ! Frequency of statistical sampling [s]
                            ! For most complete sampling, let 
                            ! stats_tsamp = dt_main
                            ! (stats_tsamp must multiply evenly into stats_tout)
...
&setfields
datafile = "../sam_benchmark_runs/DYCOMS_RF01_96x96x320/DYCOMS_RF01_96x96x320.nc"
input_type = "SAM"  ! input_type = "clubb", "coamps_les", or "SAM"
l_input_um = .true. 
l_input_vm = .true.
l_input_rtm = .true. 
...
/
```
3. Run `./run_scripts/run_scm.bash arm`, `./run_scripts/run_scm.bash astex_a209`, `./run_scripts/run_scm.bash dycoms2_rf01` and `./run_scripts/run_scm.bash gabls2`
...
1. Generate the the netcdf files with the scripts before and each time copy the output into a different directory. Give the directory the name you want to have in the legend of the plots for each of the three different results (grid adaptation, lo-res control, hi-res benchmark).

(function () {
  ...
  function getLayout() {
    return document.getElementById("tune-tab-layout");
  }

  function getDivider() {
    return document.getElementById("tune-pane-divider");
  }

  function saveWidth(width) {
    try {
      window.localStorage.setItem("tune-ui-right-width", String(Math.round(width)));
    } catch (e) {}
  }

  function loadWidth() {
    try {
      var raw = window.localStorage.getItem("tune-ui-right-width");
      if (!raw) return null;
      var num = Number(raw);

...
distribution of variables, and $f(\mathbf{x})$ is a function of some or all of
these variables. In other words, we want to compute the average value of
$f(\mathbf{x})$ over the entire domain.
...
\begin{equation}
\zeta_j = \int \mathbf{1}_j(\mathbf{x}) P(\mathbf{x}) d\mathbf{x}
\end{equation}
...
points in the category $C_j$ is $S_j$ rather than $\zeta_j$. In order to do this,
we define a new function:
\begin{equation}
L(\mathbf{x}) = \frac{\zeta_j}{S_j}\ \mathrm{where}\ \mathbf{x}\in C_j
\end{equation}
...
\right)d\mathbf{x} =
\int f(\mathbf{x}) L(\mathbf{x}) H(\mathbf{x}) d\mathbf{x}
\label{SILHS integral rewrite}
\end{equation}
where
\begin{equation}
H(\mathbf{x}) = \frac{P(\mathbf{x})}{L(\mathbf{x})}
\end{equation}

Here, $H(\mathbf{x})$ is the new PDF that has been resized to fit our prescribed
probabililties. We can easily verify this:
\begin{align}
\int \mathbf{1}_j(\mathbf{x}) H(\mathbf{x}) d\mathbf{x} &=
\int \mathbf{1}_j(\mathbf{x}) \frac{P(\mathbf{x})}{L(\mathbf{x})} d\mathbf{x} \\
 &= \frac{S_j}{\zeta_j} \int \mathbf{1}_j(\mathbf{x}) P(\mathbf{x}) d\mathbf{x} \\
 &= \frac{S_j}{\zeta_j} \zeta_j \\
 &= S_j
\end{align}

Now, instead of drawing points from the $P(\mathbf{x})$ distribution and
evaluate the function $f(\mathbf{x})$, we draw points from the $H(\mathbf{x})$
distribution and evaluate the function $f(\mathbf{x}) L(\mathbf{x})$.

Based on the integral form given in \eqref{SILHS integral rewrite}, each sample
$f(\mathbf{x}_i)$ needs to be multiplied by $L(\mathbf{x}_i)$. This factor is
referred to as the sample point ``weight''. Each sample point has a weight,
denoted as:
\begin{equation}
\omega_i = L(\mathbf{x}_i) = \frac{\zeta_j}{S_j}\ \mathrm{where}\ \mathbf{x}_i\in
...
\end{equation}
where $\mathbf{x}_i$ is the $i$\textsuperscript{th} sample point. The integral
gives the mean of $f(\mathbf{x})$ over the entire PDF. To determine the mean
over some subset $\Omega$ of the PDF, the following formula should be used:
\begin{equation}
\int f(\mathbf{x})\mathbf{1}_\Omega(\mathbf{x}) P(\mathbf{x}) d\mathbf{x}
\approx \frac{1}{N}\sum_{i=1}^{N_s} \omega_i
f(\mathbf{x}_i) \mathbf{1}_\Omega(\mathbf{x}_i)
\end{equation}
However, note that this equation will implicitly be scaled by $\zeta_\Omega$. To
compute and unscaled version of the integral, the following could be used
instead:
\begin{equation}
\frac{1}{\zeta_\Omega}
\int f(\mathbf{x})\mathbf{1}_\Omega(\mathbf{x}) P(\mathbf{x}) d\mathbf{x}
\approx \frac{1}{\zeta_\Omega}\frac{1}{N}\sum_{i=1}^{N_s} \omega_i
f(\mathbf{x}_i) \mathbf{1}_\Omega(\mathbf{x}_i)
\end{equation}
...
by:
\begin{equation}
v = \int\left[f(\mathbf{x})L(\mathbf{x})\right]^2 H(\mathbf{x})d\mathbf{x}
- \mu^2
\end{equation}

We can split the integral up over our importance categories.
\begin{align}
v &= \sum_{j=1}^{N_c} \left\{ \int\left[f(\mathbf{x})L(\mathbf{x})\right]^2
\mathbf{1}_j(\mathbf{x}) H(\mathbf{x})
d\mathbf{x} \right\} - \mu^2 \\
&= \sum_{j=1}^{N_c} \left\{\left(\frac{\zeta_j}{S_j}\right)^2
\int\left[f(\mathbf{x})\right]^2 \mathbf{1}_j(\mathbf{x})
\left(\frac{S_j}{\zeta_j}\right) P(\mathbf{x})\ d\mathbf{x} \right\} - \mu^2 \\
&= \sum_{j=1}^{N_c} \left\{ \left(\frac{\zeta_j}{S_j}\right)
\int\left[f(\mathbf{x})\right]^2\mathbf{1}_j(\mathbf{x}) P(\mathbf{x})\
...
let $u_j=\int\left[f(\mathbf{x})\right]^2\mathbf{1}_j(\mathbf{x})P(\mathbf{x})\
d\mathbf{x}$. Then we write \eqref{SILHS importance variance simplified} as
\begin{equation}
v = \sum_{j=1}^{N_c} \frac{\zeta_j u_j}{S_j} - \mu^2
\end{equation}
The probabilities $S_j$ must add to one, so we can write this as:
\begin{equation}
v = \sum_{j=1}^{N_c-1} \frac{\zeta_j u_j}{S_j} + \frac{\zeta_{N_c}
...
\begin{gather}
\frac{\partial v}{\partial S_j} = -\frac{\zeta_j u_j}{S_j^2} + \frac{\zeta_{N_c}
u_{N_c}}{\left(1 - \sum_{j=1}^{N_c-1} S_j\right)^2} = 0 \\
\frac{\zeta_j u_j}{S_j^2} = \frac{\zeta_{N_c}
u_{N_c}}{\left(1 - \sum_{j=1}^{N_c-1} S_j\right)^2} \\
\frac{\zeta_j u_j}{S_j^2} = \frac{\zeta_{N_c}u_{N_c}}{S_{N_c}^2} \\
\frac{\sqrt{\zeta_j u_j}}{S_j} = \frac{\sqrt{\zeta_{N_c}u_{N_c}}}{S_{N_c}}
\end{gather}

So, in each category, the sample probability $S_j$ should be proportional to
$\sqrt{\zeta_j u_j}$. If we let
\begin{equation}
\alpha = \frac{S_j}{\sqrt{\zeta_j u_j}} \label{alpha def}
\end{equation}
...
\begin{equation}
S_j = \frac{\sqrt{\zeta_j u_j}}{\sum_{j^*=1}^{N_c}\sqrt{\zeta_{j^*} u_{j^*}}}
\end{equation}

...
\end{equation}
where $\mathbf{x}_i$ is the $i$th sample point. To integrate over some subset
$\Omega$ of the PDF, the following formula should be used:
\begin{equation}
\int_\Omega f(\mathbf{x}) P(\mathbf{x}) d\mathbf{x} \approx \frac{\displaystyle\frac{1}{N}
\sum_{i=1}^{N_s} \omega_i f(\mathbf{x}_i) \mathbf{1}_\Omega(\mathbf{x}_i)}

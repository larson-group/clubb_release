...
\begin{verbatim}
 xyzzy(1:gr%nzmax) = variable_arr1(1:gr%nzmax) * variable_scl1 &
                    * variable_arr2(1:gr%nzmax)**2
\end{verbatim}
...
specificaton uses one, include a comment on that line saying:
\begin{verbatim}
  ! known magic number
\end{verbatim}
This will cause the check\_for\_errors.py script from saying that there is a 
...
\begin{verbatim}
  ! known magic flag
\end{verbatim}
This will prevent the check\_for\_errors.py script from saying that there is a
...
\item  Append a grid label to a variable name when the variable has been interpolated to a different grid.  For instance, wp2 is natively on the zm grid.  When it is interpolated to the zt grid, we call it ``wp2\_zt."  However, when wp2 is kept on the zm, we simply call it ``wp2" without appending ``\_zm."

...
\item When writing new source code, use fixed format rather than free format.  However, 
...
are not used already.  Currently prognostic\_variables is used in \texttt{statistics.F}
and \texttt{hoc.F}, while diagnostic\_variables is used in \texttt{hoc.F}, \texttt{statistics.F}, and 
\texttt{parameterization\_interface.F}.  Global variables like these make debugging 
...
allows for 132, but the number 100 was chosen to keep the line length 
within reason.  \newline

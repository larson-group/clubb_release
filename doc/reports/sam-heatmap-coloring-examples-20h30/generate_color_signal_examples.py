#!/usr/bin/env python3
"""Render probability plus five matched raw-SAM color-signal panels.
...
# ARM's CLUBB case clock starts at 41,400 s, while the raw SAM filenames are
# elapsed LES seconds.  This reproduces the Plot tab's midpoint alignment for
# the visible 20.5 h / one-minute CLUBB window.
CLUBB_WINDOW_START_SECONDS = int(20.5 * 3600)
CLUBB_WINDOW_DURATION_SECONDS = 60
CLUBB_MODEL_TIME_INITIAL_SECONDS = 41_400
SAM_TARGET_SECONDS = int(
    CLUBB_WINDOW_START_SECONDS
    + 0.5 * CLUBB_WINDOW_DURATION_SECONDS
    - CLUBB_MODEL_TIME_INITIAL_SECONDS
)
TARGET_HEIGHT_M = 1900.0
BINS = 72
...
def main() -> int:
    ...
    snapshot = load_snapshot(DEFAULT_SAM_RUN, SAM_TARGET_SECONDS, TARGET_HEIGHT_M)
    ...
    figure.suptitle(
        f"ARM CLUBB time: {CLUBB_WINDOW_START_SECONDS / 3600.0:.1f} h / {snapshot.height_m:.0f} m — raw SAM elapsed {snapshot.elapsed_minutes:.0f} min; probability plus five color signals",
        color="#f8fafc",
        fontsize=14,
        y=0.98,
    )
    figure.text(
        0.5,
        0.012,
        "Upper left is probability only. All covariance panels center w and q. The final panel is the third-order kernel w′rₜ′², so it is not a covariance. Each signed panel uses its own robust 99.5th-percentile magnitude cap.",
        ha="center",
        color="#a9b8cd",
        fontsize=9,
    )
    ...
    metadata = {
        ...
        "clubb_window_start_seconds": CLUBB_WINDOW_START_SECONDS,
        "clubb_window_duration_seconds": CLUBB_WINDOW_DURATION_SECONDS,
        "clubb_model_time_initial_seconds": CLUBB_MODEL_TIME_INITIAL_SECONDS,
        "sam_target_seconds": SAM_TARGET_SECONDS,

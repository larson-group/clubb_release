...
module pdf_9_module
  ...
  subroutine diagnose_pdf9_mixing_reach(                              &
    ...
    ! diagnostic allocation.  The current NetCDF statistics API accepts
    ! scalar, profile, and full 2-D field samples; the per-parcel arrays below
    ! are 3-D and therefore remain internal diagnostics rather than stats
    ! variables.
    if ( stats%l_sample ) then

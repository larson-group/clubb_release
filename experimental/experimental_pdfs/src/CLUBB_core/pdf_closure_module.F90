...
module pdf_closure_module
  ...
  use model_flags, only: &
      ...
      iiPDF_Bergmann,   & ! Bergmann three-component Gaussian PDF
      iiPDF_parcel_ensemble, & ! Parcel-weighted G3 plus residual ADG1 pair
      iiPDF_trivariate_transport ! Moment-driven trivariate transport 2G PDF
...
!------------------------------------------------------------------------
  ...
  subroutine pdf_closure( nz, ngrdcol, sclr_dim, sclr_tol, gr,        &
    ...
    use parameter_indices, only: &
        ...
        itrivar_moisture_tail_gain,     &
        itrivar_center_budget,          &
        itrivar_w_direction_scale,      &
        itrivar_g1_wrt_capture,         &
        itrivar_plume_structure_strength
    ...
    use trivariate_transport_pdf, only: &
        ...
        diagnose_transport_implicit_responders ! Procedure(s)
    ...
    elseif ( iiPDF_type == iiPDF_trivariate_transport ) then

      ! PDF 10 diagnoses the complete w-r_t-theta_l component covariance
      ! matrices, including distinct G1 and G2 correlations.
      call trivariate_transport_pdf_driver( nz, ngrdcol,                           & ! In
                                            wm, rtm, thlm,                          & ! In
                                            wp2, rtp2, thlp2,                       & ! In
                                            wprtp, wpthlp, rtpthlp,                 & ! In
                                            Skw, Skrt, Skthl,                       & ! In
                                            clubb_params(:,itrivar_moisture_tail_gain), & ! In
                                            clubb_params(:,itrivar_center_budget),   & ! In
                                            clubb_params(:,itrivar_w_direction_scale), & ! In
                                            clubb_params(:,itrivar_g1_wrt_capture),  & ! In
                                            clubb_params(:,itrivar_plume_structure_strength), & ! In
                                            pdf_params%w_1, pdf_params%w_2,         & ! Out
                                            pdf_params%rt_1, pdf_params%rt_2,       & ! Out
                                            pdf_params%thl_1, pdf_params%thl_2,     & ! Out
                                            ...
                                            pdf_params%mixt_frac,                   & ! Out
                                            pdf_params%corr_w_rt_1,                 & ! Out
                                            pdf_params%corr_w_rt_2,                 & ! Out
                                            pdf_params%corr_w_thl_1,                & ! Out
                                            pdf_params%corr_w_thl_2,                & ! Out
                                            pdf_params%corr_rt_thl_1,               & ! Out
                                            pdf_params%corr_rt_thl_2 )                ! Out

      ! PDF 10 only changes the trivariate thermodynamic state.  Until a
      ! separate u/v/passive-scalar third-moment diagnosis is introduced, keep
      ! those responder means and widths equal to their grid values.  Their
      ! w correlations below retain the supplied second moments.
      !$acc parallel loop gang vector collapse(2) default(present)
      do k = 1, nz
        do i = 1, ngrdcol
          u_1(i,k) = um(i,k)
          ...
          v_1(i,k) = vm(i,k)
          v_2(i,k) = vm(i,k)
          varnce_u_1(i,k) = up2(i,k)
          varnce_u_2(i,k) = up2(i,k)
          varnce_v_1(i,k) = vp2(i,k)
          varnce_v_2(i,k) = vp2(i,k)
          alpha_u(i,k) = one_half
          alpha_v(i,k) = one_half
        end do
      end do
      !$acc end parallel loop
      ...
      if ( l_scalar_calc ) then
        ...
        do sclr = 1, sclr_dim
          do k = 1, nz
            do i = 1, ngrdcol
              sclr1(i,k,sclr) = sclrm(i,k,sclr)
              sclr2(i,k,sclr) = sclrm(i,k,sclr)
              varnce_sclr1(i,k,sclr) = sclrp2(i,k,sclr)
              varnce_sclr2(i,k,sclr) = sclrp2(i,k,sclr)
              alpha_sclr(i,k,sclr) = one_half
            end do
          end do
        end do
      ...
      call calc_comp_corrs_binormal( nz, ngrdcol,                              & ! In
                                     ...
                                     varnce_u_1, varnce_u_2,                    & ! In
                                     pdf_params%varnce_w_1,                     & ! In
                                     pdf_params%varnce_w_2,                     & ! In
                                     pdf_params%mixt_frac,                      & ! In
                                     corr_u_w_1, corr_u_w_2 )                     ! Out

      call calc_comp_corrs_binormal( nz, ngrdcol,                              & ! In
                                     vpwp, vm, wm,                              & ! In
                                     v_1, v_2,                                  & ! In
                                     pdf_params%w_1, pdf_params%w_2,            & ! In
                                     varnce_v_1, varnce_v_2,                    & ! In
                                     pdf_params%varnce_w_1,                     & ! In
                                     pdf_params%varnce_w_2,                     & ! In
                                     pdf_params%mixt_frac,                      & ! In
                                     corr_v_w_1, corr_v_w_2 )                     ! Out
    ...
    ! PDF 10 provides its own unequal component correlations.  Other PDF
    ! types use the historical equal-correlation reconstruction.
    ...
    elseif ( iiPDF_type == iiPDF_trivariate_transport ) then
      ...
      ! The PDF 10 driver has already diagnosed the w-r_t and w-theta_l
      ! component correlations together with the component covariance matrices.
    ...
    if ( l_explicit_turbulent_adv_xpyp .or. &
         iiPDF_type == iiPDF_trivariate_transport .or. &
    ...
    ! trivariate transport PDF freezes responders diagnosed from its directly integrated
    ! higher-order moments.  These coefficients let the existing CLUBB
    ! semi-implicit turbulent-advection operators advance w'^3, r_t'^2,
    ! theta_l'^2, and r_t'theta_l' without replacing trivariate transport PDF's component
    ! geometry by an ADG1 responder assumption.
    if ( iiPDF_type == iiPDF_trivariate_transport ) then
      call diagnose_transport_implicit_responders( nz, ngrdcol, &
                                                    wp2, rtp2, thlp2, rtpthlp, & ! In
                                                    wp4, wprtp2, wpthlp2, wprtpthlp, & ! In
                                                    pdf_implicit_coefs_terms%coef_wp4_implicit, & ! Out
                                                    pdf_implicit_coefs_terms%coef_wprtp2_implicit, & ! Out
                                                    pdf_implicit_coefs_terms%term_wprtp2_explicit, & ! Out
                                                    pdf_implicit_coefs_terms%coef_wpthlp2_implicit, & ! Out
                                                    pdf_implicit_coefs_terms%term_wpthlp2_explicit, & ! Out
                                                    pdf_implicit_coefs_terms%coef_wprtpthlp_implicit, & ! Out
                                                    pdf_implicit_coefs_terms%term_wprtpthlp_explicit ) ! Out

      ! Passive scalars remain on their established explicit PDF path in this
      ! first conversion.  Store their directly integrated moments as explicit
      ! residuals so that the shared semi-implicit dispatcher preserves that
      ! behavior rather than silently dropping their turbulent advection.
      if ( l_scalar_calc ) then
        pdf_implicit_coefs_terms%coef_wpsclrp2_implicit(:,:,:) = zero
        pdf_implicit_coefs_terms%term_wpsclrp2_explicit(:,:,:) = wpsclrp2(:,:,:)
        pdf_implicit_coefs_terms%coef_wprtpsclrp_implicit(:,:,:) = zero
        pdf_implicit_coefs_terms%term_wprtpsclrp_explicit(:,:,:) = wpsclrprtp(:,:,:)
        pdf_implicit_coefs_terms%coef_wpthlpsclrp_implicit(:,:,:) = zero
        pdf_implicit_coefs_terms%term_wpthlpsclrp_explicit(:,:,:) = wpsclrpthlp(:,:,:)
      end if
    end if
  ...
  subroutine pdf_closure_3G( nz, ngrdcol, sclr_dim, sclr_tol, gr,     &
                          hydromet_dim, p_in_Pa, exner, thv_ds,       &
                          wm, wp2, wp3, Skw,                          &
                          rtm, rtp2, wprtp,                           &
                          thlm, thlp2, wpthlp,                        &
    ...
    ! Description:
    ! Diagnoses a three-Gaussian closure and integrates its higher moments,
    ! cloud quantities, buoyancy terms, and turbulent transport terms.
    ! Bergmann diagnoses all components instantaneously.  The parcel-ensemble
    ! option supplies component 3 and diagnoses an exact residual ADG1 pair.
    !
    ! Based of the original formulation, but with some tweaks
    ! to remove some of the less realistic assumptions and
    ! improve transport terms.
    ...
    use parameter_indices, only: &
        ...
        ibergmann_delta,               &
        ibergmann_lambda_w,            &
        ibergmann_lambda_scalar,       &
        ibergmann_lambda_w_scalar,     &
        ibergmann_lambda_rt_thl
    ...
    use model_flags, only: &
        iiPDF_Bergmann,          & ! Bergmann three-Gaussian PDF
        iiPDF_parcel_ensemble, & ! Parcel-weighted G3 plus residual ADG1 pair
        l_explicit_turbulent_adv_xpyp ! Variable(s)
    ...
    integer, intent(in) :: &
      iiPDF_type    ! Must select a supported three-Gaussian closure.
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nz) :: &
      ...
      alpha_u,       & ! Factor relating to normalized variance for u        [-]
      alpha_v,       & ! Factor relating to normalized variance for v        [-]
      varnce_u_3,    & ! Variance of u (3rd PDF component)             [m^2/s^2]
      varnce_v_3,    & ! Variance of v (3rd PDF component)             [m^2/s^2]
      covar_w_u_3,   & ! Covariance of w and u (3rd PDF component)     [m^2/s^2]
      covar_w_v_3      ! Covariance of w and v (3rd PDF component)     [m^2/s^2]
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nz) :: &
      ...
      vprcp_contrib_comp_1,   & ! <v'rc'> contrib. (1st PDF comp.)  [m/s(kg/kg)]
      vprcp_contrib_comp_2,   & ! <v'rc'> contrib. (2nd PDF comp.)  [m/s(kg/kg)]
      wprcp_contrib_comp_3,   & ! <w'rc'> contrib. (3rd PDF comp.)  [m/s(kg/kg)]
      wp2rcp_contrib_comp_3,  & ! <w'^2rc'> contrib. (3rd comp) [m^2/s^2(kg/kg)]
      rtprcp_contrib_comp_3,  & ! <rt'rc'> contrib. (3rd PDF comp.) [kg^2/kg^2]
      thlprcp_contrib_comp_3, & ! <thl'rc'> contrib. (3rd PDF comp.) [K(kg/kg)]
      uprcp_contrib_comp_3,   & ! <u'rc'> contrib. (3rd PDF comp.) [m/s(kg/kg)]
      vprcp_contrib_comp_3      ! <v'rc'> contrib. (3rd PDF comp.) [m/s(kg/kg)]

    ! variables for computing ice cloud fraction
    real( kind = core_rknd), dimension(ngrdcol,nz) :: &
      rc_1_ice, rc_2_ice, rc_3_ice
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nz) :: &
      ...
      rsatl_3, &
      rtpthlp_pdf, &
      rtm_outer,   & ! Mean rt of the residual two-Gaussian mixture [kg/kg]
      thlm_outer     ! Mean thl of the residual two-Gaussian mixture [K]
    ...
    if ( iiPDF_type /= iiPDF_Bergmann &
         .and. iiPDF_type /= iiPDF_parcel_ensemble ) then
      write(fstderr,*) "Fatal error: pdf_closure_3G received an unsupported PDF type."
    ...
    !$acc enter data create( u_1, u_2, varnce_u_1, varnce_u_2, v_1, v_2, &
    !$acc                 varnce_v_1, varnce_v_2, alpha_u, alpha_v, &
    !$acc                 varnce_u_3, varnce_v_3, covar_w_u_3, covar_w_v_3, &
    !$acc                 corr_u_w_1, corr_u_w_2, corr_v_w_1, corr_v_w_2, &
    !$acc                 tl1, tl2, tl3, &
    !$acc                 wprcp_contrib_comp_1, wprcp_contrib_comp_2, &
    !$acc                 wp2rcp_contrib_comp_1, wp2rcp_contrib_comp_2, &
    !$acc                 rtprcp_contrib_comp_1, rtprcp_contrib_comp_2, &
    !$acc                 thlprcp_contrib_comp_1, thlprcp_contrib_comp_2, &
    !$acc                 uprcp_contrib_comp_1, uprcp_contrib_comp_2, &
    !$acc                 vprcp_contrib_comp_1, vprcp_contrib_comp_2, &
    !$acc                 wprcp_contrib_comp_3, wp2rcp_contrib_comp_3, &
    !$acc                 rtprcp_contrib_comp_3, thlprcp_contrib_comp_3, &
    !$acc                 uprcp_contrib_comp_3, vprcp_contrib_comp_3, &
    !$acc                 rc_1_ice, rc_2_ice, rc_3_ice, &
    !$acc                 rsatl_1, rsatl_2, rsatl_3, rtpthlp_pdf, &
    !$acc                 rtm_outer, thlm_outer )
    ...
    ! Initialize quantities that Bergmann diagnoses from the input moments.
    do k = 1, nz
      do i = 1, ngrdcol
        pdf_params%alpha_thl(i,k) = one_half
        pdf_params%alpha_rt(i,k) = one_half
    ...
    select case ( iiPDF_type )
    case ( iiPDF_Bergmann )

      ! Diagnose the memoryless Bergmann trinormal PDF.
      call bergmann_pdf_driver( nz, ngrdcol, sclr_dim, sclr_tol,                  & ! In
                                wm, rtm, thlm, um, vm,                            & ! In
                                wp2, wp3, rtp2, thlp2, up2, vp2,                  & ! In
                                wprtp, wpthlp, rtpthlp, upwp, vpwp,                & ! In
                                sigma_sqd_w, clubb_params(:,ibeta),               & ! In
                                mixt_frac_max_mag,                                & ! In
                                clubb_params(:,ibergmann_delta),                   & ! In
                                clubb_params(:,ibergmann_lambda_w),                & ! In
                                clubb_params(:,ibergmann_lambda_scalar),           & ! In
                                clubb_params(:,ibergmann_lambda_w_scalar),         & ! In
                                clubb_params(:,ibergmann_lambda_rt_thl),           & ! In
                                sclrm, sclrp2, wpsclrp, l_scalar_calc,             & ! In
                                err_info,                                          & ! InOut
                                pdf_params%w_1, pdf_params%w_2,                   & ! Out
                                pdf_params%rt_1, pdf_params%rt_2,                 & ! Out
                                ...
                                sclr1, sclr2, varnce_sclr1,                       & ! Out
                                varnce_sclr2, alpha_sclr,                         & ! Out
                                pdf_params%w_3, pdf_params%rt_3,                  & ! Out
                                pdf_params%thl_3, pdf_params%varnce_w_3,          & ! Out
                                pdf_params%varnce_rt_3,                           & ! Out
                                pdf_params%varnce_thl_3,                          & ! Out
                                pdf_params%corr_w_rt_3,                           & ! Out
                                pdf_params%corr_w_thl_3,                          & ! Out
                                pdf_params%corr_rt_thl_3,                         & ! Out
                                pdf_params%mixt_frac_3, rtpthlp_pdf,              & ! Out
                                varnce_u_3, varnce_v_3,                           & ! Out
                                covar_w_u_3, covar_w_v_3 )                          ! Out

    case ( iiPDF_parcel_ensemble )
      ...
      call pdf_9_driver(                                           &
          ...
          pdf_params%varnce_rt_1, pdf_params%varnce_rt_2,            & ! Out
          pdf_params%varnce_thl_1, pdf_params%varnce_thl_2,          & ! Out
          varnce_u_1, varnce_u_2, varnce_v_1, varnce_v_2,           & ! Out
          pdf_params%mixt_frac,                                      & ! Out
          pdf_params%alpha_rt, pdf_params%alpha_thl, alpha_u, alpha_v, & ! Out
          sclr1, sclr2, varnce_sclr1, varnce_sclr2, alpha_sclr,      & ! Out
          pdf_params%w_3, pdf_params%rt_3, pdf_params%thl_3,         & ! Out
          pdf_params%varnce_w_3, pdf_params%varnce_rt_3,             & ! Out
          pdf_params%varnce_thl_3, pdf_params%corr_w_rt_3,           & ! Out
          pdf_params%corr_w_thl_3, pdf_params%corr_rt_thl_3,         & ! Out
          pdf_params%mixt_frac_3, rtpthlp_pdf,                       & ! Out
          varnce_u_3, varnce_v_3,                                    & ! Out
          covar_w_u_3, covar_w_v_3,                                  & ! Out
          pdf_params%pdf9_candidate_valid )                            ! In
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        if ( one - pdf_params%mixt_frac_3(i,k) > tiny(one) ) then
          rtm_outer(i,k) = ( rtm(i,k) - pdf_params%mixt_frac_3(i,k) &
                             * pdf_params%rt_3(i,k) ) &
                           / ( one - pdf_params%mixt_frac_3(i,k) )
          thlm_outer(i,k) = ( thlm(i,k) - pdf_params%mixt_frac_3(i,k) &
                              * pdf_params%thl_3(i,k) ) &
                            / ( one - pdf_params%mixt_frac_3(i,k) )
        else
          rtm_outer(i,k) = rtm(i,k)
          thlm_outer(i,k) = thlm(i,k)
        end if
      end do
    end do
    ...
    call calc_comp_corrs_binormal( nz, ngrdcol,                                           & ! In
                                   rtpthlp_pdf, rtm_outer, thlm_outer,                    & ! In
                                   pdf_params%rt_1, pdf_params%rt_2,                      & ! In
                                   pdf_params%thl_1, pdf_params%thl_2,                    & ! In
                                   pdf_params%varnce_rt_1, pdf_params%varnce_rt_2,        & ! In
                                   pdf_params%varnce_thl_1, pdf_params%varnce_thl_2,      & ! In
    ...
    ! Both supported residual diagnoses use ADG1 outer components.  Their
    ! w-scalar and w-wind covariance is carried by component separation.
    !$acc parallel loop gang vector collapse(2) default(present)
    do k = 1, nz
      do i = 1, ngrdcol
        pdf_params%corr_w_rt_1(i,k)  = zero
    ...
    ! The calls above integrate the conditional outer pair.  Scale that pair
    ! by its total weight and add the third Gaussian about the grid mean.
    ! Component 3 has grid-mean horizontal winds because no separate u_3 or
    ! v_3 means are diagnosed, but its w-u and w-v covariances may be nonzero.
    !$acc parallel loop gang vector collapse(2) default(present)
    do k = 1, nz
      do i = 1, ngrdcol
        wp2rtp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wp2rtp(i,k) &
                      + pdf_params%mixt_frac_3(i,k) &
                        * ( ( pdf_params%rt_3(i,k) - rtm(i,k) ) &
                            * ( ( pdf_params%w_3(i,k) - wm(i,k) )**2 &
                                + pdf_params%varnce_w_3(i,k) ) &
                            + 2.0_core_rknd * ( pdf_params%w_3(i,k) - wm(i,k) ) &
                              * pdf_params%corr_w_rt_3(i,k) &
                              * sqrt( pdf_params%varnce_w_3(i,k) &
                                      * pdf_params%varnce_rt_3(i,k) ) )
        wp2thlp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wp2thlp(i,k) &
                       + pdf_params%mixt_frac_3(i,k) &
                         * ( ( pdf_params%thl_3(i,k) - thlm(i,k) ) &
                             * ( ( pdf_params%w_3(i,k) - wm(i,k) )**2 &
                                 + pdf_params%varnce_w_3(i,k) ) &
                             + 2.0_core_rknd * ( pdf_params%w_3(i,k) - wm(i,k) ) &
                               * pdf_params%corr_w_thl_3(i,k) &
                               * sqrt( pdf_params%varnce_w_3(i,k) &
                                       * pdf_params%varnce_thl_3(i,k) ) )
        wp2up(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wp2up(i,k) &
                     + pdf_params%mixt_frac_3(i,k) * 2.0_core_rknd &
                       * ( pdf_params%w_3(i,k) - wm(i,k) ) * covar_w_u_3(i,k)
        wpup2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wpup2(i,k) &
                     + pdf_params%mixt_frac_3(i,k) &
                       * ( pdf_params%w_3(i,k) - wm(i,k) ) * varnce_u_3(i,k)
        wpvp2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wpvp2(i,k) &
                     + pdf_params%mixt_frac_3(i,k) &
                       * ( pdf_params%w_3(i,k) - wm(i,k) ) * varnce_v_3(i,k)
        wp2up2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wp2up2(i,k) &
                       + pdf_params%mixt_frac_3(i,k) &
                         * ( ( ( pdf_params%w_3(i,k) - wm(i,k) )**2 &
                               + pdf_params%varnce_w_3(i,k) ) * varnce_u_3(i,k) &
                             + 2.0_core_rknd * covar_w_u_3(i,k)**2 )
        wp2vp2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wp2vp2(i,k) &
                       + pdf_params%mixt_frac_3(i,k) &
                         * ( ( ( pdf_params%w_3(i,k) - wm(i,k) )**2 &
                               + pdf_params%varnce_w_3(i,k) ) * varnce_v_3(i,k) &
                             + 2.0_core_rknd * covar_w_v_3(i,k)**2 )
        wp4(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wp4(i,k) &
                   + pdf_params%mixt_frac_3(i,k) &
                     * ( ( pdf_params%w_3(i,k) - wm(i,k) )**4 &
                         + 6.0_core_rknd * ( pdf_params%w_3(i,k) - wm(i,k) )**2 &
                           * pdf_params%varnce_w_3(i,k) &
                         + three * pdf_params%varnce_w_3(i,k)**2 )
        wprtp2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wprtp2(i,k) &
                      + pdf_params%mixt_frac_3(i,k) &
                        * ( ( pdf_params%w_3(i,k) - wm(i,k) ) &
                            * ( ( pdf_params%rt_3(i,k) - rtm(i,k) )**2 &
                                + pdf_params%varnce_rt_3(i,k) ) &
                            + 2.0_core_rknd * ( pdf_params%rt_3(i,k) - rtm(i,k) ) &
                              * pdf_params%corr_w_rt_3(i,k) &
                              * sqrt( pdf_params%varnce_w_3(i,k) &
                                      * pdf_params%varnce_rt_3(i,k) ) )
        wpthlp2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wpthlp2(i,k) &
                       + pdf_params%mixt_frac_3(i,k) &
                         * ( ( pdf_params%w_3(i,k) - wm(i,k) ) &
                             * ( ( pdf_params%thl_3(i,k) - thlm(i,k) )**2 &
                                 + pdf_params%varnce_thl_3(i,k) ) &
                             + 2.0_core_rknd * ( pdf_params%thl_3(i,k) - thlm(i,k) ) &
                               * pdf_params%corr_w_thl_3(i,k) &
                               * sqrt( pdf_params%varnce_w_3(i,k) &
                                       * pdf_params%varnce_thl_3(i,k) ) )
        wprtpthlp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) * wprtpthlp(i,k) &
                         + pdf_params%mixt_frac_3(i,k) &
                           * ( ( pdf_params%w_3(i,k) - wm(i,k) ) &
                               * ( pdf_params%rt_3(i,k) - rtm(i,k) ) &
                               * ( pdf_params%thl_3(i,k) - thlm(i,k) ) &
                               + ( pdf_params%w_3(i,k) - wm(i,k) ) &
                                 * pdf_params%corr_rt_thl_3(i,k) &
                                 * sqrt( pdf_params%varnce_rt_3(i,k) &
                                         * pdf_params%varnce_thl_3(i,k) ) &
                               + ( pdf_params%rt_3(i,k) - rtm(i,k) ) &
                                 * pdf_params%corr_w_thl_3(i,k) &
                                 * sqrt( pdf_params%varnce_w_3(i,k) &
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        tl1(i,k)  = pdf_params%thl_1(i,k)*exner(i,k)
        tl2(i,k)  = pdf_params%thl_2(i,k)*exner(i,k)
        tl3(i,k)  = pdf_params%thl_3(i,k)*exner(i,k)
      end do
    end do
    !$acc end parallel loop
    ...
    rsatl_2 = sat_mixrat_liq_api( nz, ngrdcol, gr, p_in_Pa, tl2, &
                                  saturation_formula  ) ! h1g, 2010-06-16 end mod
    rsatl_3 = sat_mixrat_liq_api( nz, ngrdcol, gr, p_in_Pa, tl3, saturation_formula )
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        pdf_params%rsatl_1(i,k) = rsatl_1(i,k)
        pdf_params%rsatl_2(i,k) = rsatl_2(i,k)
        pdf_params%rsatl_3(i,k) = rsatl_3(i,k)
      end do
    end do
    !$acc end parallel loop
    ...
    if ( l_calc_ice_supersat_frac ) then
      ...
      call transform_pdf_chi_eta_component( nz, ngrdcol,                         & ! In
                                            tl3, pdf_params%rsatl_3,             & ! In
                                            pdf_params%rt_3, exner,              & ! In
                                            pdf_params%varnce_thl_3,             & ! In
                                            pdf_params%varnce_rt_3,              & ! In
                                            pdf_params%corr_rt_thl_3,            & ! In
                                            pdf_params%chi_3,                    & ! In
                                            pdf_params%crt_3, pdf_params%cthl_3, & ! Out
                                            pdf_params%stdev_chi_3,              & ! Out
                                            pdf_params%stdev_eta_3,              & ! Out
                                            pdf_params%covar_chi_eta_3,          & ! Out
                                            pdf_params%corr_chi_eta_3 )            ! Out

      call calc_liquid_cloud_frac_component( nz, ngrdcol,                        & ! In
                                             pdf_params%chi_3,                   & ! In
                                             pdf_params%stdev_chi_3,             & ! In
                                             pdf_params%cloud_frac_3,            & ! Out
                                             pdf_params%rc_3 )                     ! Out

      call calc_ice_cloud_frac_component( nz, ngrdcol,                           & ! In
                                          pdf_params%chi_3,                      & ! In
                                          pdf_params%stdev_chi_3,                & ! In
                                          pdf_params%rc_3,                       & ! In
                                          pdf_params%cloud_frac_3, p_in_Pa, tl3, & ! In
                                          pdf_params%rsatl_3, pdf_params%crt_3,  & ! In
                                          saturation_formula,                   & ! In
                                          pdf_params%ice_supersat_frac_3,        & ! Out
                                          rc_3_ice )                               ! Out
      ...
      do k = 1, nz
        do i = 1, ngrdcol
          ice_supersat_frac(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                                   * ( pdf_params%mixt_frac(i,k) &
                                       * pdf_params%ice_supersat_frac_1(i,k) &
                                       + ( one - pdf_params%mixt_frac(i,k) ) &
                                         * pdf_params%ice_supersat_frac_2(i,k) ) &
                                   + pdf_params%mixt_frac_3(i,k) &
                                     * pdf_params%ice_supersat_frac_3(i,k)
        end do
      end do
      !$acc end parallel loop
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        cloud_frac(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                         * ( pdf_params%mixt_frac(i,k) * pdf_params%cloud_frac_1(i,k) &
                             + ( one - pdf_params%mixt_frac(i,k) ) &
                               * pdf_params%cloud_frac_2(i,k) ) &
                         + pdf_params%mixt_frac_3(i,k) * pdf_params%cloud_frac_3(i,k)
        rcm(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                   * ( pdf_params%mixt_frac(i,k) * pdf_params%rc_1(i,k) &
                       + ( one - pdf_params%mixt_frac(i,k) ) * pdf_params%rc_2(i,k) ) &
                   + pdf_params%mixt_frac_3(i,k) * pdf_params%rc_3(i,k)
        rcm(i,k) = max( zero_threshold, rcm(i,k) )
      end do
    end do
    !$acc end parallel loop
    ...
    pdf_params%corr_w_chi_3 &
    = calc_corr_chi_x( pdf_params%crt_3, pdf_params%cthl_3, &
                       sqrt(pdf_params%varnce_rt_3), &
                       sqrt(pdf_params%varnce_thl_3), &
                       pdf_params%stdev_chi_3, pdf_params%corr_w_rt_3, &
                       pdf_params%corr_w_thl_3 )
    ...
    pdf_params%corr_w_eta_3 &
    = calc_corr_eta_x( pdf_params%crt_3, pdf_params%cthl_3, &
                       sqrt(pdf_params%varnce_rt_3), &
                       sqrt(pdf_params%varnce_thl_3), &
                       pdf_params%stdev_eta_3, pdf_params%corr_w_rt_3, &
                       pdf_params%corr_w_thl_3 )
    ...
    call calc_xprcp_component( nz, ngrdcol,                                          & ! In
                               wm, rtm, thlm, um, vm, rcm,                           & ! In
                               pdf_params%w_3, pdf_params%rt_3,                      & ! In
                               pdf_params%thl_3, um, vm,                             & ! In
                               pdf_params%varnce_w_3, pdf_params%chi_3,              & ! In
                               pdf_params%stdev_chi_3, pdf_params%stdev_eta_3,       & ! In
                               pdf_params%corr_w_chi_3, pdf_params%corr_chi_eta_3,   & ! In
                               pdf_params%crt_3, pdf_params%cthl_3,                  & ! In
                               pdf_params%rc_3, pdf_params%cloud_frac_3, iiPDF_type, & ! In
                               wprcp_contrib_comp_3, wp2rcp_contrib_comp_3,          & ! Out
                               rtprcp_contrib_comp_3, thlprcp_contrib_comp_3,        & ! Out
                               uprcp_contrib_comp_3, vprcp_contrib_comp_3 )            ! Out
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        ...
        ! Calculate <w'rc'>, <w'^2 rc'>, <rt'rc'>, and <thl'rc'>.
        wprcp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                     * ( pdf_params%mixt_frac(i,k) * wprcp_contrib_comp_1(i,k) &
                         + ( one - pdf_params%mixt_frac(i,k) ) &
                           * wprcp_contrib_comp_2(i,k) ) &
                     + pdf_params%mixt_frac_3(i,k) * wprcp_contrib_comp_3(i,k)

        wp2rcp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                      * ( pdf_params%mixt_frac(i,k) * wp2rcp_contrib_comp_1(i,k) &
                          + ( one - pdf_params%mixt_frac(i,k) ) &
                            * wp2rcp_contrib_comp_2(i,k) ) &
                      + pdf_params%mixt_frac_3(i,k) * wp2rcp_contrib_comp_3(i,k)

        rtprcp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                      * ( pdf_params%mixt_frac(i,k) * rtprcp_contrib_comp_1(i,k) &
                          + ( one - pdf_params%mixt_frac(i,k) ) &
                            * rtprcp_contrib_comp_2(i,k) ) &
                      + pdf_params%mixt_frac_3(i,k) * rtprcp_contrib_comp_3(i,k)

        thlprcp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                       * ( pdf_params%mixt_frac(i,k) * thlprcp_contrib_comp_1(i,k) &
                           + ( one - pdf_params%mixt_frac(i,k) ) &
                             * thlprcp_contrib_comp_2(i,k) ) &
                       + pdf_params%mixt_frac_3(i,k) * thlprcp_contrib_comp_3(i,k)

        uprcp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                     * ( pdf_params%mixt_frac(i,k) * uprcp_contrib_comp_1(i,k) &
                         + ( one - pdf_params%mixt_frac(i,k) ) &
                           * uprcp_contrib_comp_2(i,k) ) &
                     + pdf_params%mixt_frac_3(i,k) * uprcp_contrib_comp_3(i,k)

        vprcp(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                     * ( pdf_params%mixt_frac(i,k) * vprcp_contrib_comp_1(i,k) &
                         + ( one - pdf_params%mixt_frac(i,k) ) &
                           * vprcp_contrib_comp_2(i,k) ) &
                     + pdf_params%mixt_frac_3(i,k) * vprcp_contrib_comp_3(i,k)
      end do
    end do
    !$acc end parallel loop
    ...
    do k = 1,nz
      do i = 1, ngrdcol
        rcp2(i,k) = ( one - pdf_params%mixt_frac_3(i,k) ) &
                    * ( pdf_params%mixt_frac(i,k) &
                        * ( pdf_params%chi_1(i,k)*pdf_params%rc_1(i,k) &
                            + pdf_params%cloud_frac_1(i,k) &
                              * pdf_params%stdev_chi_1(i,k)**2 ) &
                        + ( one-pdf_params%mixt_frac(i,k) ) &
                          * ( pdf_params%chi_2(i,k)*pdf_params%rc_2(i,k) &
                              + pdf_params%cloud_frac_2(i,k) &
                                * pdf_params%stdev_chi_2(i,k)**2 ) ) &
                    + pdf_params%mixt_frac_3(i,k) &
                      * ( pdf_params%chi_3(i,k)*pdf_params%rc_3(i,k) &
                          + pdf_params%cloud_frac_3(i,k) &
                            * pdf_params%stdev_chi_3(i,k)**2 ) &
                    - rcm(i,k)**2
        rcp2(i,k) = max( zero_threshold, rcp2(i,k) )
    ...
    if ( var_on_stats_list( stats, "w_up_in_cloud" ) &
         .or. var_on_stats_list( stats, "w_down_in_cloud" ) ) then
                 
      call calc_w_up_in_cloud_3( nz, ngrdcol, &                                        ! In
                               pdf_params%mixt_frac, pdf_params%mixt_frac_3, &          ! In
                               pdf_params%cloud_frac_1, pdf_params%cloud_frac_2, &      ! In
                               pdf_params%cloud_frac_3, &                               ! In
                               pdf_params%w_1, pdf_params%w_2, pdf_params%w_3, &        ! In
                               pdf_params%varnce_w_1, pdf_params%varnce_w_2, &          ! In
                               pdf_params%varnce_w_3, &                                 ! In
                               w_up_in_cloud, w_down_in_cloud, &                       ! Out
                               cloudy_updraft_frac, cloudy_downdraft_frac )            ! Out
  ...
  !=============================================================================
  subroutine calc_wpxp3_pdf( nz, ngrdcol,             &
                             wm, xm, w_1, w_2,        &
                             x_1, x_2,                &
                             varnce_w_1, varnce_w_2,  &
                             varnce_x_1, varnce_x_2,  &
    ...
    ! Description:
    !   Calculates <w'x'^3> by analytically integrating a two-component
    !   bivariate normal PDF.  This is the turbulent-transport flux in the
    !   prognostic scalar-third-moment equation.  For each component, with
    !   d_w = mu_w-<w>, d_x = mu_x-<x>, and c_wx = cov(w,x), the contribution is
    !
    !   d_w * ( d_x^3 + 3 d_x sigma_x^2 )
    !   + 3 c_wx * ( d_x^2 + sigma_x^2 ).
    !
    !   The expression is exact for the Gaussian component and retains both
    !   center separation and internal w-x covariance contributions.
    !---------------------------------------------------------------------------
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nz), intent(in) :: &
      ...
      xm,         & ! Grid-mean scalar [units vary]
      w_1,        & ! G1 mean vertical velocity [m/s]
      w_2,        & ! G2 mean vertical velocity [m/s]
      x_1,        & ! G1 mean scalar [units vary]
      x_2,        & ! G2 mean scalar [units vary]
      varnce_w_1, & ! G1 w variance [m^2/s^2]
      varnce_w_2, & ! G2 w variance [m^2/s^2]
      varnce_x_1, & ! G1 scalar variance [(units vary)^2]
      varnce_x_2, & ! G2 scalar variance [(units vary)^2]
      corr_w_x_1, & ! G1 w-scalar correlation [-]
      corr_w_x_2, & ! G2 w-scalar correlation [-]
      mixt_frac     ! G1 mixture fraction [-]

    !-------------------------- Output Variables ---------------------------
    real( kind = core_rknd ), dimension(ngrdcol,nz), intent(out) :: &
      wpxp3    ! <w'x'^3> [m/s (units vary)^3]

    !---------------------------- Local Variables --------------------------
    real( kind = core_rknd ) :: &
      dw_1, & ! G1 w mean departure [m/s]
      dw_2, & ! G2 w mean departure [m/s]
      dx_1, & ! G1 scalar mean departure [units vary]
      dx_2, & ! G2 scalar mean departure [units vary]
      cov_wx_1, & ! G1 internal w-scalar covariance [m/s units vary]
      cov_wx_2    ! G2 internal w-scalar covariance [m/s units vary]
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        ...
        dx_1 = x_1(i,k) - xm(i,k)
        dx_2 = x_2(i,k) - xm(i,k)
        cov_wx_1 = corr_w_x_1(i,k) * sqrt( max( varnce_w_1(i,k), 0.0_core_rknd ) &
                                                  * max( varnce_x_1(i,k), 0.0_core_rknd ) )
        cov_wx_2 = corr_w_x_2(i,k) * sqrt( max( varnce_w_2(i,k), 0.0_core_rknd ) &
                                                  * max( varnce_x_2(i,k), 0.0_core_rknd ) )

        wpxp3(i,k) = mixt_frac(i,k) * ( &
          dw_1 * ( dx_1**3 + three * dx_1 * varnce_x_1(i,k) ) &
          + three * cov_wx_1 * ( dx_1**2 + varnce_x_1(i,k) ) ) &
          + ( one - mixt_frac(i,k) ) * ( &
          dw_2 * ( dx_2**3 + three * dx_2 * varnce_x_2(i,k) ) &
          + three * cov_wx_2 * ( dx_2**2 + varnce_x_2(i,k) ) )
      end do
    end do
    !$acc end parallel loop
...
!     corr_v_w_i,     & ! Correlation of v and w (ith PDF component)   [-]
  ...
  subroutine calc_w_up_in_cloud( nz, ngrdcol, &
    ...
    ! Description:
    !   Backward-compatible two-component interface for callers outside this
    !   module.  The implementation delegates to the general three-component
    !   Gaussian-mixture calculation with a zero third-component weight.
    !-----------------------------------------------------------------------------
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nz), intent(in) :: &
      mixt_frac, &     ! Conditional mixture fraction [-]
      cloud_frac_1, &  ! Cloud fraction in component 1 [-]
      cloud_frac_2, &  ! Cloud fraction in component 2 [-]
      w_1, &           ! Mean vertical velocity in component 1 [m/s]
      w_2, &           ! Mean vertical velocity in component 2 [m/s]
      varnce_w_1, &    ! Vertical-velocity variance in component 1 [m2/s2]
      varnce_w_2       ! Vertical-velocity variance in component 2 [m2/s2]

    real( kind = core_rknd ), dimension(ngrdcol,nz), intent(out) :: &
      w_up_in_cloud, &        ! Mean cloudy-updraft velocity [m/s]
      w_down_in_cloud, &      ! Mean cloudy-downdraft velocity [m/s]
      cloudy_updraft_frac, &  ! Cloudy-updraft fraction [-]
      cloudy_downdraft_frac   ! Cloudy-downdraft fraction [-]

    real( kind = core_rknd ), dimension(ngrdcol,nz) :: &
      zero_component ! Zero-valued third-component fields [units vary]

    zero_component = zero

    call calc_w_up_in_cloud_3( nz, ngrdcol, &                             ! In
                               mixt_frac, zero_component, &               ! In
                               cloud_frac_1, cloud_frac_2, zero_component,& ! In
                               w_1, w_2, zero_component, &                 ! In
                               varnce_w_1, varnce_w_2, zero_component, &   ! In
                               w_up_in_cloud, w_down_in_cloud, &          ! Out
  ...
  !=============================================================================
  subroutine calc_w_up_in_cloud_3( nz, ngrdcol, &
                                 mixt_frac, mixt_frac_3, &
                                 cloud_frac_1, cloud_frac_2, cloud_frac_3, &
                                 w_1, w_2, w_3, &
                                 varnce_w_1, varnce_w_2, varnce_w_3, &
                                 w_up_in_cloud, w_down_in_cloud, &
                                 cloudy_updraft_frac, cloudy_downdraft_frac )
    ...
    !
    ! The formulas are valid for the Gaussian-mixture PDFs in CLUBB (ADG1,
    ! ADG2, new hybrid, and Bergmann), hence we omit calculation if another
    ! PDF type is used.
    !
    ...
    !----------- Local Variables -----------
    real( kind = core_rknd ) :: &
      w_up_1, w_up_2, w_up_3, &          ! Integral of w where w > 0
      w_down_1, w_down_2, w_down_3, &    ! Integral of w where w < 0
      stdev_w_1, stdev_w_2, stdev_w_3, & ! Standard deviation of w
      ratio_w_1, &             ! mu_w_1 / ( sqrt(2) * sigma_w_1 )
      ratio_w_2, &             ! mu_w_2 / ( sqrt(2) * sigma_w_2 )
      ratio_w_3, &             ! mu_w_3 / ( sqrt(2) * sigma_w_3 )
      erf_ratio_w_1, &         ! erf( ratio_w_1 )
      erf_ratio_w_2, &         ! erf( ratio_w_2 )
      erf_ratio_w_3, &         ! erf( ratio_w_3 )
      exp_neg_ratio_w_1_sqd, & ! exp( -ratio_w_1^2 )
      exp_neg_ratio_w_2_sqd, & ! exp( -ratio_w_2^2 )
      exp_neg_ratio_w_3_sqd, & ! exp( -ratio_w_3^2 )
      updraft_frac_1, &        ! Fraction of 1st PDF comp. where w > 0
      updraft_frac_2, &        ! Fraction of 2nd PDF comp. where w > 0
      updraft_frac_3, &        ! Fraction of 3rd PDF comp. where w > 0
      downdraft_frac_1, &      ! Fraction of 1st PDF comp. where w < 0
      downdraft_frac_2, &      ! Fraction of 2nd PDF comp. where w < 0
      downdraft_frac_3, &      ! Fraction of 3rd PDF comp. where w < 0
      weight_1, weight_2       ! Absolute weights of outer PDF components [-]
    ...
    do k = 1, nz
      do i = 1, ngrdcol
        ...
        stdev_w_3 = sqrt(varnce_w_3(i,k))
        weight_1 = ( one - mixt_frac_3(i,k) ) * mixt_frac(i,k)
        weight_2 = ( one - mixt_frac_3(i,k) ) * ( one - mixt_frac(i,k) )
        ...
        ! Calculate quantities in the 3rd PDF component.  For two-component
        ! PDFs, mixt_frac_3 is zero and this contribution drops out exactly.
        if ( w_3(i,k) > max_num_stdevs * stdev_w_3 ) then
          w_up_3 = w_3(i,k)
          updraft_frac_3 = one
          w_down_3 = zero
          downdraft_frac_3 = zero
        elseif ( w_3(i,k) < - max_num_stdevs * stdev_w_3 ) then
          w_up_3 = zero
          updraft_frac_3 = zero
          w_down_3 = w_3(i,k)
          downdraft_frac_3 = one
        else
          ratio_w_3 = w_3(i,k) / ( sqrt_2 * max(eps, stdev_w_3) )
          erf_ratio_w_3 = erf( ratio_w_3 )
          exp_neg_ratio_w_3_sqd = exp( -ratio_w_3**2 )

          w_up_3 = one_half * w_3(i,k) * ( one + erf_ratio_w_3 ) &
                   + ( stdev_w_3 / sqrt_2pi ) * exp_neg_ratio_w_3_sqd
          updraft_frac_3 = one_half * ( one + erf_ratio_w_3 )
          w_down_3 = one_half * w_3(i,k) * ( one - erf_ratio_w_3 ) &
                     - ( stdev_w_3 / sqrt_2pi ) * exp_neg_ratio_w_3_sqd
          downdraft_frac_3 = one - updraft_frac_3
        end if

        ! Calculate the total cloudy updraft fraction.
        cloudy_updraft_frac(i,k) &
        = weight_1 * cloud_frac_1(i,k) * updraft_frac_1 &
          + weight_2 * cloud_frac_2(i,k) * updraft_frac_2 &
          + mixt_frac_3(i,k) * cloud_frac_3(i,k) * updraft_frac_3

        ! Calculate the total cloudy downdraft fraction.
        cloudy_downdraft_frac(i,k) &
        = weight_1 * cloud_frac_1(i,k) * downdraft_frac_1 &
          + weight_2 * cloud_frac_2(i,k) * downdraft_frac_2 &
          + mixt_frac_3(i,k) * cloud_frac_3(i,k) * downdraft_frac_3

        ! Calculate the mean vertical velocity found in a cloudy updraft.
        w_up_in_cloud(i,k) &
        = ( weight_1 * cloud_frac_1(i,k) * w_up_1 &
            + weight_2 * cloud_frac_2(i,k) * w_up_2 &
            + mixt_frac_3(i,k) * cloud_frac_3(i,k) * w_up_3 ) &
          / max( eps, cloudy_updraft_frac(i,k) )

        ! Calculate the mean vertical velocity found in a cloudy downdraft.
        w_down_in_cloud(i,k) &
        = ( weight_1 * cloud_frac_1(i,k) * w_down_1 &
            + weight_2 * cloud_frac_2(i,k) * w_down_2 &
            + mixt_frac_3(i,k) * cloud_frac_3(i,k) * w_down_3 ) &
  ...
  end subroutine calc_w_up_in_cloud_3
  ...
  subroutine pdf_closure_driver( gr, nzm, nzt, ngrdcol,                   & ! Intent(in)
                                 ...
                                 wp2sclrp, wpsclrp2,                      & ! Intent(out)
                                 wpsclrprtp, wpsclrpthlp,                  & ! Intent(out)
                                 pdf9_update_state,                       & ! Optional intent(in)
                                 pdf9_l_implemented, pdf9_lmin )           ! Optional intent(in)
    ...
    logical, intent(in), optional :: &
      pdf9_update_state,  & ! Diagnose the next PDF-9 candidate after closure [-]
      pdf9_l_implemented    ! CLUBB is embedded in a host model [-]

    real( kind = core_rknd ), intent(in), optional :: &
      pdf9_lmin ! PDF-9 parcel-reach minimum length [m]
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nzt) :: &
      ...
      pdf9_thvm           ! Current PDF-9 virtual potential temperature [K]
    ...
    real( kind = core_rknd ), dimension(ngrdcol,nzt) :: &
      ...
      wp2rcp,       & ! w'^2 r_c' (on thermo. grid)     [(m^2 kg)/(s^2 kg)]
    ...
    !$acc                    wpsclrp2_zm, wpsclrpthlp_zm, wp2sclrp_zm, sclrprcp )

    !$acc enter data if( hydromet_dim > 0 ) create( wphydrometp_zt )

    !---------------------------------------------------------------------------
    ! Interpolate wp3 to momentum levels, rtp3 and thlp3 to momentum levels
    ! as needed, and
    ! wp2, rtp2, thlp2, up2, vp2, and sclrp2 to thermodynamic levels, and then
    ! compute Skw for both the momentum and thermodynamic grid levels. Skrt and
    ! Skthl are computed for thermodynamic grid levels and for momentum levels
    ! as needed. Sku, Skv, and Sksclr are computed only for thermodynamic levels here.
    !---------------------------------------------------------------------------

    wp2_zt(:,:) = zm2zt_api( nzm, nzt, ngrdcol, gr, wp2(:,:), w_tol_sqd ) ! Positive definite
    ...
    thlp2_zt(:,:) = zm2zt_api( nzm, nzt, ngrdcol, gr, thlp2(:,:), thl_tol**2 ) ! Positive definite
    call Skx_func( nzt, ngrdcol, thlp2_zt, thlp3, &
                   thl_tol, clubb_params, &
                   Skthl_zt )  
                   
    rtp2_zt(:,:) = zm2zt_api( nzm, nzt, ngrdcol, gr, rtp2(:,:),  rt_tol**2 ) ! Positive definite
    call Skx_func( nzt, ngrdcol, rtp2_zt, rtp3, &
                   rt_tol, clubb_params, &
                   Skrt_zt )
                   
    up2_zt(:,:) = zm2zt_api( nzm, nzt, ngrdcol, gr, up2(:,:), w_tol_sqd ) ! Positive definite
    call Skx_func( nzt, ngrdcol, up2_zt, up3, &
                   w_tol, clubb_params, &
                   Sku_zt )   
                      
    vp2_zt(:,:) = zm2zt_api( nzm, nzt, ngrdcol, gr, vp2(:,:), w_tol_sqd ) ! Positive definite                
    call Skx_func( nzt, ngrdcol, vp2_zt, vp3, &
                   w_tol, clubb_params, &
                   Skv_zt )   

    if ( l_call_pdf_closure_twice .or. ( stats%l_sample .and. l_samp_stats_in_pdf_call ) ) then
    ...
    ! Call closure scheme
    !----------------------------------------------------------------
    if ( iiPDF_type == iiPDF_Bergmann .or. iiPDF_type == iiPDF_parcel_ensemble ) then

      call pdf_closure_3G( nzt, ngrdcol, sclr_dim, sclr_tol, gr, & ! intent(in)
             hydromet_dim, p_in_Pa, exner, thv_ds_zt,         & ! intent(in)
             wm_zt, wp2_zt, wp3, Skw_zt,                      & ! intent(in)
    ...
    if ( iiPDF_type == iiPDF_Bergmann ) then

      ! Bergmann supplies the full three-Gaussian <w'^4> directly.  Freeze its
      ! local shape factor during the advancement so that
      !
      !   <w'^4>^(n+1) ~= K_B^n <w'^2>^n <w'^2>^(n+1),
      !   K_B^n = <w'^4>^n / <w'^2>^n^2.
      !
      ! This is the same semi-implicit linearization used by the trivariate transport PDF wp3
      ! turbulent-advection operator.  It introduces no new closure solve and
      ! preserves the diagnosed fourth moment at the frozen PDF state.
      !$acc parallel loop gang vector collapse(2) default(present)
      do k = 1, nzt
        do i = 1, ngrdcol
          ! Do not form a fourth-moment ratio in an unresolved w-variance
          ! state.  Besides being physically meaningless, wp2**2 can underflow
          ! before the division on quiet levels even when wp2 itself is positive.
          if ( wp2_zt(i,k) > w_tol_sqd ) then
            pdf_implicit_coefs_terms%coef_wp4_implicit(i,k) = &
              max( wp4_zt(i,k) / ( wp2_zt(i,k) * wp2_zt(i,k) ), zero )
          else
    ...
    if ( l_call_pdf_closure_twice ) then
      ...
      wp4(:,:) = zt2zm_api( nzm, nzt, ngrdcol, gr, wp4_zt(:,:), zero_threshold ) ! Pos. def. 
    ...
    else if ( stats%l_sample .and. l_samp_stats_in_pdf_call ) then
    ...
    if ( stats%l_sample .and. l_samp_stats_in_pdf_call ) then
      ...
      !$acc update host( rc_coef, wp2rcp, rtprcp, rcp2, Skw_velocity, &
      !$acc              cloud_frac_zm, ice_supersat_frac_zm, rtm_zm, thlm_zm, &
      !$acc              rcm_zm, rcm_supersat_adj, cloud_frac, ice_supersat_frac, &
      !$acc              rcm_in_layer, cloud_cover, sigma_sqd_w, wpthvp, wp2thvp, &
      !$acc              wp2up, rtpthvp, thlpthvp, wprcp, rc_coef_zm, thlprcp, &
      !$acc              wpup2, wpvp2, wp2up2, wp2vp2, wp4, wp2rtp, wprtp2, &
      !$acc              wp2thlp, wpthlp2, wprtpthlp, w_up_in_cloud, &
      !$acc              w_down_in_cloud, cloudy_updraft_frac, cloudy_downdraft_frac, &
      !$acc              pdf_params%w_1, pdf_params%w_2, &
      !$acc              pdf_params%varnce_w_1, pdf_params%varnce_w_2, &
      !$acc              pdf_params%rt_1, pdf_params%rt_2, &
      !$acc              pdf_params%varnce_rt_1, pdf_params%varnce_rt_2, &
      ...
      !$acc              pdf_params%alpha_thl, pdf_params%alpha_rt, &
      !$acc              pdf_params%crt_1, pdf_params%crt_2, pdf_params%cthl_1, &
      !$acc              pdf_params%cthl_2, pdf_params%chi_1, pdf_params%chi_2, &
      !$acc              pdf_params%stdev_chi_1, pdf_params%stdev_chi_2, &
      !$acc              pdf_params%stdev_eta_1, pdf_params%stdev_eta_2, &
      !$acc              pdf_params%covar_chi_eta_1, pdf_params%covar_chi_eta_2, &
      !$acc              pdf_params%corr_w_chi_1, pdf_params%corr_w_chi_2, &
      !$acc              pdf_params%corr_w_eta_1, pdf_params%corr_w_eta_2, &
      !$acc              pdf_params%corr_chi_eta_1, pdf_params%corr_chi_eta_2, &
      !$acc              pdf_params%rsatl_1, pdf_params%rsatl_2, &
      !$acc              pdf_params%rc_1, pdf_params%rc_2, &
      !$acc              pdf_params%cloud_frac_1, pdf_params%cloud_frac_2, &
      !$acc              pdf_params%mixt_frac, pdf_params%ice_supersat_frac_1, &
      !$acc              pdf_params%ice_supersat_frac_2, &
      !$acc              pdf_params%w_3, pdf_params%varnce_w_3, &
      !$acc              pdf_params%rt_3, pdf_params%varnce_rt_3, &
      !$acc              pdf_params%thl_3, pdf_params%varnce_thl_3, &
      !$acc              pdf_params%corr_w_rt_3, pdf_params%corr_w_thl_3, &
      !$acc              pdf_params%corr_rt_thl_3, pdf_params%chi_3, &
      !$acc              pdf_params%stdev_chi_3, pdf_params%rc_3, &
      !$acc              pdf_params%cloud_frac_3, pdf_params%mixt_frac_3 )
      ...
      call stats_update( "mixt_frac_3", pdf_params%mixt_frac_3, stats )
      call stats_update( "w_3", pdf_params%w_3, stats )
      call stats_update( "varnce_w_3", pdf_params%varnce_w_3, stats )
      call stats_update( "rt_3", pdf_params%rt_3, stats )
      call stats_update( "varnce_rt_3", pdf_params%varnce_rt_3, stats )
      call stats_update( "thl_3", pdf_params%thl_3, stats )
      call stats_update( "varnce_thl_3", pdf_params%varnce_thl_3, stats )
      call stats_update( "corr_w_rt_3", pdf_params%corr_w_rt_3, stats )
      call stats_update( "corr_w_thl_3", pdf_params%corr_w_thl_3, stats )
      call stats_update( "corr_rt_thl_3", pdf_params%corr_rt_thl_3, stats )
      call stats_update( "corr_w_chi_3", pdf_params%corr_w_chi_3, stats )
      call stats_update( "crt_3", pdf_params%crt_3, stats )
      call stats_update( "cthl_3", pdf_params%cthl_3, stats )
      call stats_update( "chi_3", pdf_params%chi_3, stats )
      call stats_update( "stdev_chi_3", pdf_params%stdev_chi_3, stats )
      call stats_update( "rc_3", pdf_params%rc_3, stats )
      call stats_update( "cloud_frac_3", pdf_params%cloud_frac_3, stats )

      if ( var_on_stats_list( stats, "chi" ) .or.       &
           var_on_stats_list( stats, "chip2" ) ) then
        chi = ( 1.0_core_rknd - pdf_params%mixt_frac_3 ) &
              * ( pdf_params%mixt_frac * pdf_params%chi_1 &
                  + ( 1.0_core_rknd - pdf_params%mixt_frac ) * pdf_params%chi_2 ) &
              + pdf_params%mixt_frac_3 * pdf_params%chi_3
        call stats_update( "chi", chi, stats )
      end if

      if ( var_on_stats_list( stats, "chip2" ) ) then
        if ( iiPDF_type == iiPDF_Bergmann .or. iiPDF_type == iiPDF_parcel_ensemble ) then
          chip2 = ( 1.0_core_rknd - pdf_params%mixt_frac_3 ) &
                  * ( pdf_params%mixt_frac &
                      * ( pdf_params%stdev_chi_1**2 + ( pdf_params%chi_1 - chi )**2 ) &
                      + ( 1.0_core_rknd - pdf_params%mixt_frac ) &
                        * ( pdf_params%stdev_chi_2**2 + ( pdf_params%chi_2 - chi )**2 ) ) &
                  + pdf_params%mixt_frac_3 &
                    * ( pdf_params%stdev_chi_3**2 + ( pdf_params%chi_3 - chi )**2 )
        else
      ...
      if ( sclr_dim > 0 ) then
        !$acc update host( sclrpthvp, sclrprcp, wp2sclrp, wpsclrp2, &
        !$acc              wpsclrprtp, wpsclrpthlp )
        do sclr = 1, sclr_dim
          write( sclr_idx, * ) sclr
    ...
    ! PDF 9 consumes the candidate that existed on entry, completes all
    ! thermodynamic integration and statistics, and only then diagnoses the
    ! candidate for the next selected closure call.  This keeps candidate
    ! transport out of the same PDF call that created it.
    if ( iiPDF_type == iiPDF_parcel_ensemble &
         .and. present(pdf9_update_state) ) then
      if ( pdf9_update_state ) then
        if ( .not. present(pdf9_l_implemented) .or. .not. present(pdf9_lmin) ) then
          write(fstderr,*) "Fatal error: PDF 9 state update lacks parcel-reach inputs."
          err_info%err_code = clubb_fatal_error
          return
        end if

        ! rcm has just been integrated by this closure call.  Recompute thvm
        ! here so the next-candidate parcel solve uses the same thermodynamic
        ! state, independent of whether this is the pre- or post-advance PDF
        ! placement.
        call calculate_thvm( nzt, ngrdcol,                         &
                             thlm, rtm, rcm, exner, thv_ds_zt,     & ! In
                             pdf9_thvm )                            ! Out

        call diagnose_pdf9_mixing_reach(                              &
            nzm, nzt, ngrdcol, gr, pdf9_l_implemented,                 & ! In
            p_in_Pa, exner, rtm, thlm, pdf9_thvm, wp2, wprtp, wpthlp, & ! In
            thv_ds_zt, pdf9_lmin, clubb_params, saturation_formula,   & ! In
            stats, err_info,                                            & ! InOut
            pdf_params%w_3, pdf_params%rt_3, pdf_params%thl_3,          & ! InOut
            pdf_params%varnce_w_3, pdf_params%varnce_rt_3,              & ! InOut
            pdf_params%varnce_thl_3, pdf_params%corr_w_rt_3,            & ! InOut
            pdf_params%corr_w_thl_3, pdf_params%corr_rt_thl_3,          & ! InOut
            pdf_params%mixt_frac_3,                                     & ! In
            pdf_params%pdf9_candidate_valid,                           & ! InOut
  ...
  subroutine pdf_closure_driver_zm( gr, nzm, nzt, ngrdcol, hydromet_dim,       & ! Intent(in)
    ...
    use model_flags, only: &
        iiPDF_new,        & ! new PDF
        iiPDF_new_hybrid, & ! new hybrid PDF
        iiPDF_trivariate_transport ! trivariate transport PDF
    ...
    ! trivariate transport PDF also supplies frozen turbulent-advection responders.  Initialize
    ! the local momentum-grid copy whenever a closure uses that shared type.
    if ( iiPDF_type == iiPDF_new .or. iiPDF_type == iiPDF_new_hybrid .or. &
         iiPDF_type == iiPDF_trivariate_transport ) then
    ...
    if ( iiPDF_type == iiPDF_Bergmann .or. iiPDF_type == iiPDF_parcel_ensemble ) then
      ...
      call pdf_closure_3G( nzm, ngrdcol, sclr_dim, sclr_tol, gr,      & ! intent(in)
             hydromet_dim, p_in_Pa_zm, exner_zm, thv_ds_zm,        & ! intent(in)
             wm_zm, wp2, wp3_zm, Skw_zm,                           & ! intent(in)

...
module advance_windm_edsclrm_module
  ...
  subroutine advance_windm_edsclrm &
             ( dt, wm_zt, Km_zm, Kmb_zm, ug, vg, um_ref, vm_ref, &
    ...
    real( kind = core_rknd ), dimension(gr%nz), intent(in) ::  &
      ...
      Kmb_zm,          & ! Eddy diffusivity of themo on momentum levels [m^s/s]
...
!HPF$ INDEPENDENT
      do i = 1, edsclr_dim
        rhs(1:gr%nz,i)  &
        = windm_edsclrm_rhs( windm_edsclrm_scalar, dt, dummy_nu, Kmb_zm, &            ! in
...
!HPF$ INDEPENDENT, REDUCTION(wpedsclrp)
      forall( i = 1:edsclr_dim )
        wpedsclrp(2:gr%nz-1,i) = &
          - 0.5_core_rknd * xpwp_fnc( Kmb_zm(2:gr%nz-1), edsclrm(2:gr%nz-1,i), & ! in
      ...
      ! Build the left-hand side matrix.
      call windm_edsclrm_lhs( dt, dummy_nu, wm_zt, Kmb_zm, wind_speed, u_star_sqd,  & ! in
                              rho_ds_zm, invrs_rho_ds_zt,  &               ! in
                              l_implemented, l_imp_sfc_momentum_flux,  &   ! in
                              lhs )                                        ! out
